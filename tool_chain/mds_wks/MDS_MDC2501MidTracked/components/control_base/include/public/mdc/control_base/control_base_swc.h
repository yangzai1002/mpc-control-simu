...
#ifndef MDC_CONTROLBASESWC_H
#define MDC_CONTROLBASESWC_H
...
namespace ara {
namespace adsfi {
class BodyCommandServiceInterfaceServer;
}
}
namespace ara {
namespace adsfi {
...
namespace adsfi {
class PlanningResultIntfClient;
}
namespace mdc {
namespace control_base {
namespace {
using ara::adsfi::BodyCommandServiceInterfaceServer;
...
using adsfi::PlanningResultIntfClient;
using InitHandleType = std::function<bool()>;
using StopHandleType = std::function<void()>;
}

class ControlBaseSwC {
public:
    explicit ControlBaseSwC();

    virtual ~ControlBaseSwC();
    ...
    ara::core::Vector<std::string> GetBodyCommandServiceInterfaceServerVec()
    {
        std::lock_guard<std::mutex> lck(bodyCommandServiceInterfaceServerPortVecMtx_);
        return bodyCommandServiceInterfaceServerPortVec_;
    }
    ...
    ara::core::Vector<std::string> GetPlanningResultIntfClientVec()
    {
        std::lock_guard<std::mutex> lck(planningResultIntfClientPortVecMtx_);
        return planningResultIntfClientPortVec_;
    }
    

    std::shared_ptr<BodyCommandServiceInterfaceServer> GetBodyCommandServiceInterfaceServer(const ara::core::String& portName);
    ...
    std::shared_ptr<PlanningResultIntfClient> GetPlanningResultIntfClient(const ara::core::String& portName);
...
private:
    ...
    /* server agent */
    ara::core::Vector<uint32_t> bodyCommandServiceInterfaceServerIdVec_;
    ara::core::Vector<std::string> bodyCommandServiceInterfaceServerPortVec_;
    std::mutex bodyCommandServiceInterfaceServerPortVecMtx_;
    ara::core::Map<std::string, uint32_t> bodyCommandServiceInterfaceServerIdMap_;
    ara::core::Map<std::string, std::shared_ptr<BodyCommandServiceInterfaceServer>> bodyCommandServiceInterfaceServerInsMap_;
    std::mutex bodyCommandServiceInterfaceServerInsMapMtx_;
    /* client agent */
    ...
    /* client agent */
    ara::core::Vector<uint32_t> planningResultIntfClientIdVec_;
    ara::core::Vector<std::string> planningResultIntfClientPortVec_;
    std::mutex planningResultIntfClientPortVecMtx_;
    ara::core::Map<std::string, uint32_t> planningResultIntfClientIdMap_;
    ara::core::Map<std::string, std::shared_ptr<PlanningResultIntfClient>> planningResultIntfClientInsMap_;
    std::mutex planningResultIntfClientInsMapMtx_;

};
}  /* namespace control_base */
}  /* namespace mdc */

#endif  /* MDC_CONTROLBASESWC_H */