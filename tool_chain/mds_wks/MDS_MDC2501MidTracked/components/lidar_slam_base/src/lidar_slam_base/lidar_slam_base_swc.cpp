...
using LOG_SPACE = mdc::lidar_slam_base::LidarSlamBaseLogger;

LidarSlamBaseSwC::LidarSlamBaseSwC()
    : initFlag_(false),
      workFlag_(true),
...
std::shared_ptr<LocationInfoIntfServer> LidarSlamBaseSwC::GetLocationInfoIntfServer(const ara::core::String& portName)
{
...
std::shared_ptr<SensorLidarPointCloudServiceInterfaceClient> LidarSlamBaseSwC::GetSensorLidarPointCloudServiceInterfaceClient(const ara::core::String& portName)
{
...
std::shared_ptr<SensorImuServiceInterfaceClient> LidarSlamBaseSwC::GetSensorImuServiceInterfaceClient(const ara::core::String& portName)
{
...
std::shared_ptr<SensorWheelSpeedServiceInterfaceClient> LidarSlamBaseSwC::GetSensorWheelSpeedServiceInterfaceClient(const ara::core::String& portName)
{
...
bool LidarSlamBaseSwC::LoadConfig()
{
    /* 通过环境变量获取配置文件路径 */
...
bool LidarSlamBaseSwC::DoesFileExist(const ara::core::String& filePath)
{
...
bool LidarSlamBaseSwC::IsDirType(const mode_t& fileMode)
{
    if (S_ISDIR(fileMode)) {
        return true;
    }
...
bool LidarSlamBaseSwC::ParsingInstanceId(const std::unique_ptr<MdcYamlNode>& config)
{
    if (!config->HasKeyValue("lidar_slam_base")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find lidar_slam_base in config file!";
        return false;
    }
    auto swcConfig = (*(config.get()))["lidar_slam_base"];
    /* server agent */
