...
MultyCameraDetBaseSwC::MultyCameraDetBaseSwC()
    : initFlag_(false),
      workFlag_(true),
...
bool MultyCameraDetBaseSwC::Init()
{
    ...
    #endif
    /* server agent */
    for (auto id : visionSegmentServiceInterfaceServerIdMap_) {
        auto resultToken = ara::adsfi::skeleton::VisionSegmentServiceInterfaceSkeleton::Preconstruct(
        ...
        if (resultToken.HasValue()) {
            visionSegmentServiceInterfaceServerInsMap_[id.first] = std::make_shared<VisionSegmentServiceInterfaceServer>(id.second);
        } else {
            LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Skeleton preconstruct Failed!";
            return false;
        }
        RETURN_IF_INIT_SERVICE_FAILED(
            (visionSegmentServiceInterfaceServerInsMap_[id.first]!= nullptr),
            "VisionSegmentServiceInterfaceSkeleton",
            id.second);
    }
    /* client agent */
...
void MultyCameraDetBaseSwC::Stop()
{
    workFlag_ = false;
    /* server agent */
    for (auto id : visionSegmentServiceInterfaceServerIdMap_) {
        if (visionSegmentServiceInterfaceServerInsMap_[id.first]) {
            visionSegmentServiceInterfaceServerInsMap_[id.first]->Stop();
        } else {
...
std::shared_ptr<VisionSegmentServiceInterfaceServer> MultyCameraDetBaseSwC::GetVisionSegmentServiceInterfaceServer(const ara::core::String& portName)
{
    std::lock_guard<std::mutex> lck(visionSegmentServiceInterfaceServerInsMapMtx_);
    if (visionSegmentServiceInterfaceServerInsMap_.find(portName) == visionSegmentServiceInterfaceServerInsMap_.end()) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << portName << "does not exist.";
        return nullptr;
    }
    if (!(visionSegmentServiceInterfaceServerInsMap_[portName]->Init())) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << portName << " init failed.";
        return nullptr;
    }
    return visionSegmentServiceInterfaceServerInsMap_[portName];
}
std::shared_ptr<MsgImageDataListServiceInterfaceClient> MultyCameraDetBaseSwC::GetMsgImageDataListServiceInterfaceClient(const ara::core::String& portName)
{
...
bool MultyCameraDetBaseSwC::LoadConfig()
{
    /* 通过环境变量获取配置文件路径 */
...
bool MultyCameraDetBaseSwC::DoesFileExist(const ara::core::String& filePath)
{
...
bool MultyCameraDetBaseSwC::IsDirType(const mode_t& fileMode)
{
    if (S_ISDIR(fileMode)) {
        return true;
    }
...
bool MultyCameraDetBaseSwC::ParsingInstanceId(const std::unique_ptr<MdcYamlNode>& config)
{
    if (!config->HasKeyValue("multy_camera_det_base")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find multy_camera_det_base in config file!";
        return false;
    }
    auto swcConfig = (*(config.get()))["multy_camera_det_base"];
    /* server agent */
    if (!swcConfig.HasKeyValue("sendInstanceID")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find sendInstanceID in config file!";
        return false;
    }
    swcConfig["sendInstanceID"]["VisionSegmentServiceInterface"].GetValue<std::vector<uint32_t>>(
        "instanceId", visionSegmentServiceInterfaceServerIdVec_);
    StableUnique(visionSegmentServiceInterfaceServerIdVec_);
    swcConfig["sendInstanceID"]["VisionSegmentServiceInterface"].GetValue<std::vector<std::string>>(
        "portName", visionSegmentServiceInterfaceServerPortVec_);
    StableUnique(visionSegmentServiceInterfaceServerPortVec_);
    if (visionSegmentServiceInterfaceServerIdVec_.size() != visionSegmentServiceInterfaceServerPortVec_.size()) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "The port and instance mapping information is incorrect.";
        return false;
    }
    for (size_t i = 0; i < visionSegmentServiceInterfaceServerIdVec_.size(); ++i) {
        visionSegmentServiceInterfaceServerIdMap_[visionSegmentServiceInterfaceServerPortVec_[i]] = visionSegmentServiceInterfaceServerIdVec_[i];
    }
