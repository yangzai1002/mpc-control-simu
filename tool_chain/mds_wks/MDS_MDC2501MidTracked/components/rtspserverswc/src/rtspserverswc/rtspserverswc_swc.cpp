...
#include "mdc/rtspserverswc/rtspserverswc_swc.h"
...
#include "mdc/rtspserverswc/sensorimageserviceinterface_client.h"
...
RtspserverswcSwC::RtspserverswcSwC()
    : initFlag_(false),
      workFlag_(true),
...
std::shared_ptr<SensorImageServiceInterfaceClient> RtspserverswcSwC::GetSensorImageServiceInterfaceClient(const ara::core::String& portName)
{
...
bool RtspserverswcSwC::LoadConfig()
{
    /* 通过环境变量获取配置文件路径 */
...
bool RtspserverswcSwC::DoesFileExist(const ara::core::String& filePath)
{
...
bool RtspserverswcSwC::IsDirType(const mode_t& fileMode)
{
    if (S_ISDIR(fileMode)) {
        return true;
    }
...
bool RtspserverswcSwC::ParsingInstanceId(const std::unique_ptr<MdcYamlNode>& config)
{
    if (!config->HasKeyValue("rtspserverswc")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find rtspserverswc in config file!";
        return false;
    }
    auto swcConfig = (*(config.get()))["rtspserverswc"];
    /* server agent */
