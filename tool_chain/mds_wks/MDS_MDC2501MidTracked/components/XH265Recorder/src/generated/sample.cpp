...
#include <functional>
namespace mdc {
Sample::Sample(): swcPtr_(std::make_unique<mdc::xh265_recorder::Xh265RecorderSwC>()), workFlag_(true)
{}
    ...
    while (workFlag_) {
        ...
        for (const auto& portName : sensorImageServiceInterfaceClientPortVec) {
            ...
            clientPtr->RegisterEventSensorImageNotifyHandler(
                ...
                std::placeholders::_1));
            /* 获取数据，非阻塞方式 <关键接口> */
            auto oneData = clientPtr->GetEventSensorImageOneData();
            /* 获取数据，一次获取5个数据，非阻塞方式 <关键接口> */
            auto nData = clientPtr->GetEventSensorImageNdata(5U);
