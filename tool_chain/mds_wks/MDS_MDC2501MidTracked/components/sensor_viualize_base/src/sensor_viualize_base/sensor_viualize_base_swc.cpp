...
using LOG_SPACE = mdc::sensor_viualize_base::SensorViualizeBaseLogger;

SensorViualizeBaseSwC::SensorViualizeBaseSwC()
    : initFlag_(false),
      workFlag_(true),
...
bool SensorViualizeBaseSwC::Init()
{
...
void SensorViualizeBaseSwC::Stop()
{
    workFlag_ = false;
    /* server agent */
...
std::shared_ptr<SensorLidarPointCloudServiceInterfaceClient> SensorViualizeBaseSwC::GetSensorLidarPointCloudServiceInterfaceClient(const ara::core::String& portName)
{
...
std::shared_ptr<MsgImageDataListServiceInterfaceClient> SensorViualizeBaseSwC::GetMsgImageDataListServiceInterfaceClient(const ara::core::String& portName)
{
...
bool SensorViualizeBaseSwC::LoadConfig()
{
    /* 通过环境变量获取配置文件路径 */
...
bool SensorViualizeBaseSwC::DoesFileExist(const ara::core::String& filePath)
{
...
bool SensorViualizeBaseSwC::IsDirType(const mode_t& fileMode)
{
    if (S_ISDIR(fileMode)) {
        return true;
    }
...
bool SensorViualizeBaseSwC::ParsingInstanceId(const std::unique_ptr<MdcYamlNode>& config)
{
    if (!config->HasKeyValue("sensor_viualize_base")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find sensor_viualize_base in config file!";
        return false;
    }
    auto swcConfig = (*(config.get()))["sensor_viualize_base"];
    /* server agent */
