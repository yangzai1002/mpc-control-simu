...
using LOG_SPACE = mdc::fusion_semantic_map_base::FusionSemanticMapBaseLogger;

FusionSemanticMapBaseSwC::FusionSemanticMapBaseSwC()
    : initFlag_(false),
      workFlag_(true),
...
bool FusionSemanticMapBaseSwC::Init()
{
...
void FusionSemanticMapBaseSwC::Stop()
{
    workFlag_ = false;
    /* server agent */
...
std::shared_ptr<SensorLidarPointCloudServiceInterfaceClient> FusionSemanticMapBaseSwC::GetSensorLidarPointCloudServiceInterfaceClient(const ara::core::String& portName)
{
...
std::shared_ptr<LocationInfoIntfClient> FusionSemanticMapBaseSwC::GetLocationInfoIntfClient(const ara::core::String& portName)
{
...
bool FusionSemanticMapBaseSwC::LoadConfig()
{
    /* 通过环境变量获取配置文件路径 */
...
bool FusionSemanticMapBaseSwC::DoesFileExist(const ara::core::String& filePath)
{
...
bool FusionSemanticMapBaseSwC::IsDirType(const mode_t& fileMode)
{
    if (S_ISDIR(fileMode)) {
        return true;
    }
...
bool FusionSemanticMapBaseSwC::ParsingInstanceId(const std::unique_ptr<MdcYamlNode>& config)
{
    if (!config->HasKeyValue("fusion_semantic_map_base")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find fusion_semantic_map_base in config file!";
        return false;
    }
    auto swcConfig = (*(config.get()))["fusion_semantic_map_base"];
    /* server agent */
