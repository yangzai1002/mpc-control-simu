...
int AdsfiInterface::Process(const std::string &name, std::shared_ptr<ara::adsfi::MsgHafLocation> &msg) {
    ...
    msg->pose.pose.position.y = gccs.yg;
    ...
    //unsigned short 定位状态 0:无定位; 1:单点定位定 2:伪距差分 4:固定解 5:浮点解 6:SBAS 定位 7: ppp定位
    msg->v = loc_fusion_data_ptr->v;

    msg->locationState = loc_fusion_data_ptr->status;

    //unsigned short gnss状态 0:无定位; 1:单点定位定 2:伪距差分 4:固定解 5:浮点解 6:SBAS 定位 7: ppp定位
    // msg->gnssState = ;
