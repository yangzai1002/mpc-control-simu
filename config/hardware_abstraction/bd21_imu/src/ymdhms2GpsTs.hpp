#ifndef __YMDHMS2GPS_TS__
#define __YMDHMS2GPS_TS__
#include <chrono>
#include <ctime>
#include <iostream>
#include <vector>
...
public:
    ...
    // 计算指定UTC时间点的累计闰秒数
    static int getLeapSeconds(const std::chrono::system_clock::time_point& utcTime) {
        std::time_t utcSeconds = std::chrono::system_clock::to_time_t(utcTime);
        int         count = 0;
            // 闰秒列表（截至2023年7月）
        static std::vector<std::time_t> leapSeconds = {
    ...
    static std::chrono::system_clock::time_point gpsEpoch() {
        std::tm tm = {};
        tm.tm_year = 80; // 1980年
        tm.tm_mon = 0;   // 1月
        tm.tm_mday = 6;  // 6日
    ...
    static TimeWithMillis getCurrentTimeWithMillis(int diff_ms = 0) {
        // 获取当前时间点（带毫秒精度）
    ...
    static GPSTime utcToGps(int year, int month, int day, int hour, int minute, int second, int millisecond) {
        if (1900 >= year) {
            int    week = 0;
            double tow = 0;
            return {week, tow};
        ...
        // 计算GPS起始时间点
        auto gps_epoch = Ymdhms2GpsTs::gpsEpoch();
        ...
        // 计算该时刻的累计闰秒数
        int leap_seconds = Ymdhms2GpsTs::getLeapSeconds(utc_time);
    ...
    static GPSTime utcToGps(TimeWithMillis cur_ts) {
        // 转换为time_point
