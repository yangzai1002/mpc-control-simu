...
public:
	...
	int Init(const SocketConfig &config) {
		...
		std::cout << "udp_init ok, server port " << config_.port << "local port " << config_.native_port << std::endl;
		return 0;
	}
	int RecieveData(char *data, int max_len, int debug=0) {
		if(debug&32)
		{
			printf("---------------------- in recv, fd %d ----------------------\n", socket_fd_);
		}
	...
	int RecieveData2(char *data, int max_len, int debug=0) {
		if(debug&32)
		{
			printf("---------------------- in recv, fd %d ----------------------\n", socket_fd_);
		}
