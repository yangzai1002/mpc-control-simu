...
void AdsfiInterface::Init() {
    ...
    if (ptr->GetConfig("XMidTrackSendByZMQ:U-8/this", "config_path_prefix", prefix) < 0) {
        XMidTrackSendByZMQDataInteract::Instance()->_fault.add(_ERRORCODE_CONFIG_READ, "config_path_prefix");
...
void AdsfiInterface::Callback(const std::string &name, const std::shared_ptr<ara::adsfi::VehicleActControl> &msg) {
    SetInnerActMsg(msg);
    AINFO << "CALLBACK:: " << msg->lon_control.target_speed;
