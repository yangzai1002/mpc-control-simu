...
void AdsfiInterface::Callback(const std::shared_ptr<ara::adsfi::IdpVideoEncodedFrame> &ptr) {
	...
	if (_sender_rtp == nullptr) {
		...
		AINFO << "multicast ipv4 addresses of " << para_ptr->peer_host << ": ";
		for (const auto& ip : ips) {
			AINFO << ip;
		}
		if (ips.empty()) {
			AERROR << "no valid multicast ip address found";
			_ec->ec_add(_ERRORCODE_ADDRESS_RESOLVE, para_ptr->peer_host);
			return;
		}
		_ec->ec_remove(_ERRORCODE_ADDRESS_RESOLVE);
		auto& ip = ips.front();
		AINFO << "sending multicast to " << ip << ":" << para_ptr->peer_port;
		try {
			// 3. 传入组播IP创建RTP发送者（LocalH265RTPSender需支持组播UDP）
			_sender_rtp =
			    std::make_shared<LocalH265RTPSender>(para_ptr->ssrc, ptr->fps, "26.28.1.2", ip, para_ptr->peer_port);
		} catch (const std::exception& e) {
			AERROR << "ERROR: create multicast RTP sender failed: " << e.what();
			return;
		}

		// （可选）如果LocalH265RTPSender内部未设置组播选项，需手动配置Socket（补充代码）
		// 若_sender_rtp可获取内部Socket句柄，添加以下配置：
		/*
		int sock_fd = _sender_rtp->get_socket_fd(); // 假设存在该接口获取Socket FD
		if (sock_fd >= 0) {
			// 允许发送者加入组播组（可选，部分环境需要）
			struct ip_mreqn mreq;
			inet_pton(AF_INET, ip.c_str(), &mreq.imr_multiaddr);
			inet_pton(AF_INET, "0.0.0.0", &mreq.imr_address);
			mreq.imr_ifindex = 0; // 0表示使用默认网络接口，也可指定网卡索引
			if (setsockopt(sock_fd, IPPROTO_IP, IP_MULTICAST_IF, &mreq.imr_address, sizeof(mreq.imr_address)) < 0) {
				AWARN << "setsockopt IP_MULTICAST_IF failed: " << strerror(errno);
			}
			// 设置组播TTL（默认1，限制网段内传输；可调整为更大值跨路由）
			int ttl = 1;
			if (setsockopt(sock_fd, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl)) < 0) {
				AWARN << "setsockopt IP_MULTICAST_TTL failed: " << strerror(errno);
			}
			// 禁用组播回环（默认开启，发送者会收到自己发送的组播包；按需关闭）
			int loop = 0;
			if (setsockopt(sock_fd, IPPROTO_IP, IP_MULTICAST_LOOP, &loop, sizeof(loop)) < 0) {
				AWARN << "setsockopt IP_MULTICAST_LOOP failed: " << strerror(errno);
			}
		}
		*/
	} else {
		try {
			// 发送组播RTP数据（逻辑与单播一致，底层Socket已配置为组播）
			_sender_rtp->send(ptr->frame_data.data(), ptr->frame_data.size());
			AINFO << "sent multicast " << ptr->frame_data.size() << " bytes";
			...
			AERROR << "multicast send error: " << e.what();
