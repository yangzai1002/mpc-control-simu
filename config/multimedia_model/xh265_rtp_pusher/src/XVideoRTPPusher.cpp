...
#include "xcommon_util/DatetimeUtil.hpp"
#include "config_loader/custom_stack.h"
#include "log/log.h"

#include "Ec407.hpp"
#include "H264NaluFinedRTPSender.hpp"

void XVideoRTPPusher(const VideoRTPParameter& rtp_param, const VideoEncodedFrame& frame) {
    STATIC_DATA(std::shared_ptr<Ec407>, _ec, nullptr);
    STATIC_DATA(std::shared_ptr<std::atomic<double>>, _last_data_time, nullptr);

    STATIC_DATA(VideoRTPParameter, _last_para, {})
    STATIC_DATA(std::shared_ptr<H264NaluFinedRTPSender>, _sender_rtp, nullptr);
    ...
    if (0 == rtp_param.push_enabled) {
        ...
        AINFO << "new RTP param, " << rtp_param.ssrc << ", " << rtp_param.peer_port << ", " << rtp_param.peer_host;
        _sender_rtp = nullptr;
    }
    ...
    if (_sender_rtp == nullptr) {
        ...
        AINFO << "ipv4 addresses of " << rtp_param.peer_host << ": ";
        ...
        _ec->ec_remove(_ERRORCODE_ADDRESS_RESOLVE);
        auto& ip = ips.front();
        AINFO << "sending to " << ip << ":" << rtp_param.peer_port;
        try {
            _sender_rtp =
                std::make_shared<H264NaluFinedRTPSender>(rtp_param.ssrc, frame.fps, "0.0.0.0", ip, rtp_param.peer_port);
        ...
        try {
            ...
            AINFO << "sent " << frame.frame_data.size() << " bytes";
            AINFO << std::setprecision(15) << Tools::Second() << " 获取的时间: " << frame.header.time;
