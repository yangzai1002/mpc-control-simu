...
public:
    static std::shared_ptr<YUYV420UV12> white_img(int width, int height) {
        ...
        return std::shared_ptr<YUYV420UV12>(
            new YUYV420UV12(width, height, raw, true), // shared_buffer = true 不再复制
            [raw](YUYV420UV12* ptr) {
...
public:
    ...
    void MergeDataV2(std::shared_ptr<YUYV420UV12> other, int other_start_pos_left, int other_start_pos_top,
        ...
        for (size_t i = 0; i < height; i += 2) {
            ...
            for (size_t j = 0; j < width; j += 2) {
                ...
                uchar *other_uv_ptr = other_uv_buffer + int((other_y_shift / 2 + (other_start_pos_left + j * x_step))&(~1));
    ...
    const u_int8_t* get_data() const { return buffer_; }
