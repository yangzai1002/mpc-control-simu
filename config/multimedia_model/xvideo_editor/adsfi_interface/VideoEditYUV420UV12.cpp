...
void VideoEditYUV420UV12::drawPathplan(void* ptr, const int& width, const int& height, std::string color,
    ...
    VideoEditYUV420UV12::FillPolygonYUV420_MultiThread(ptr, width, height, polygon, yuv.y, yuv.u, yuv.v, 0.2);
}
...
bool VideoEditYUV420UV12::DoYUV420YU12ToImgMode(uint8_t* p_data, size_t data_len, int img_width, int img_height,
                                                int img_mode) {
    AINFO << "DoYUV420YU12ToImgMode data_len=" << data_len << ",img_width=" << img_width << ",img_height=" << img_height
          << ",img_mode" << img_mode;
    if (img_mode == 1) {
        return true;
    }
    if (img_width <= 0 || img_height <= 0) {
        AINFO << "DoYUV420YU12ToImgMode img width or height is error";
        return false;
    }
    size_t total_size = img_width * img_height * 3 / 2;
    if (total_size > data_len) {
        AINFO << "DoYUV420YU12ToImgMode data_len is shorter";
        return false;
    } else if (total_size != data_len) {
        // 数据长度不等，可能哪儿有问题
        AINFO << "DoYUV420YU12ToImgMode data len is longer";
    }
    switch (img_mode) {
        case 2:
        ...
        default:
            AINFO << "DoYUV420YU12ToImgMode unknown img mode" << img_mode;
            break;
    }
    return true;
}
...
bool VideoEditYUV420UV12::DoYUV420UV12ToGray(uint8_t* p_data, int img_width, int img_height) {
    AINFO << "DoYUV420YU12ToImgMode DoYUV420UV12ToGray";
...
bool VideoEditYUV420UV12::DoYUV420YU12ToBinaryImg(uint8_t* p_data, int img_width, int img_height) {
    AINFO << "DoYUV420YU12ToImgMode DoYUV420YU12ToBinaryImg";
