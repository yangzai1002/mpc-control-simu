...
public:
    explicit ThreadPoolColor(size_t num_threads) {
    ...
    void wait_for_all_tasks() {
        std::unique_lock<std::mutex> lock(queue_mutex);
        completion_var.wait(lock, [this] { return task_queue.empty() && active_tasks == 0; });
    }
};
#endif
        ...
        for (int r = 0; r < 256; ++r) {
            for (int g = 0; g < 256; ++g) {
                for (int b = 0; b < 256; ++b) {
                    ...
                    get_yuv_float(r, g, b, _color_table_rgb2yuyv[index], _color_table_rgb2yuyv[index + 1],
        ...
        for (int y = 0; y < 256; ++y) {
            for (int u = 0; u < 256; ++u) {
                for (int v = 0; v < 256; ++v) {
                    ...
                    get_rgb_float(y, u, v, _color_table_yuyv2rgb[index], _color_table_yuyv2rgb[index + 1],
...
public:
    explicit Color(size_t num_threads = 2) : _num_threads(num_threads) {
        _cvt_param = {
            // rgb2yuv
    ...
    inline bool get_yuv_float(const unsigned char r, const unsigned char g, const unsigned char b, unsigned char &y,
    ...
    inline bool get_rgb_float(const unsigned char y, const unsigned char u, const unsigned char v, unsigned char &r,
    ...
    inline bool get_yuv(const unsigned char r, const unsigned char g, const unsigned char b, unsigned char &y,
                        unsigned char &u, unsigned char &v) {
    ...
    inline bool get_rgb(const unsigned char y, const unsigned char u, const unsigned char v, unsigned char &r,
                        unsigned char &g, unsigned char &b) {
