...
public:
    ...
    void send(const std::uint8_t *p, std::size_t s) {
        ...
        if (nullptr == nalus) {
            ApError("h265_rtp_pusher") << "cannot find any nalu";
        ...
        for (auto &nalu : nalus->nal_units) {
            ...
            ApInfo("h265_rtp_pusher") << "nalu, type: " << type << ", offset " << nalu->offset
                                      << ", payload_length: " << nalu->length << ", payload: "
                                      << app_common::HexUtils::to_hex_string(p + nalu->offset,
                                                                             std::min(200, (std::int32_t)nalu->length));
    ...
    void send_nalu(bool is_sps_pps, std::uint8_t nalu_type, const std::uint8_t *p, std::size_t s) {
        if (s < 2) {
            ApError("h265_rtp_pusher") << "nalu less than 2";
            return;
        }
        if (!is_sps_pps) {
    ...
    void do_send(std::uint8_t *p, std::int32_t s) {
        ...
        retry:
            ...
            if (n < 0) {
                ...
                ApError("h265_rtp_pusher")
                    << "ssrc " << _ssrc << " sendto error " << app_common::StringUtils::str_error(errno);
                throw std::runtime_error("sendto error: " + app_common::StringUtils::str_error(errno));
            }

            off += n;
            ApInfo("h265_rtp_pusher") << "ssrc " << _ssrc << " sendto " << n << " bytes"
                                      << ", s: " << s << ", off: " << off;
        }
    }
