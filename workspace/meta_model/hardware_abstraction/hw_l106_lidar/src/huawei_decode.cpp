...
int HWDecoder::Init(const DecoderConfig &config, int sensor_id) {
...
int HWDecoder::SetDifopConfig(const avos::SocketConfig &socket_config) {
...
int HWDecoder::SetSupplementaryData(const ara::adsfi::SensorLidarPacket &packets_ptr) {
	// if (packets_ptr.get() == nullptr) {
	// 	return -1;
	// }
	...
	for (int i = 0; i < 480; i++) {
		for (int j = 0; j < 4; j++) {
			vertical_angle_offset_[i][j] =
			    supple_data->vertical_angle_offset[i][j];
		}
	}

	for (int i = 0; i < MAX_LIDAR_RING; i++) {
		for (int j = 0; j < 4; j++) {
			horizontal_angle_offset_[i][j] =
			    supple_data->horizontal_angle_offset[i][j];
		}
	}
...
int HWDecoder::GetPacketAngle(const char *packet) {
	...
	if (packet_angle->msg_head.GetID() != 0x0031 ||
		...
		ApError("parse") << "Response msg is error...";
		return -1;
	}
	...
	ApInfo("input") << "Receive index: " << index;
	int nRet = -1;
...
void HWDecoder::ConvertPacktsToPoint(const ara::adsfi::CommonUdpPacket &packet, ara::adsfi::MsgLidarFrame &point_cloud, double &first_pkg_ts, int &pub_logic, bool udp_test) {
	...
	if(fabs(cur_ts-time_stamp)>0.5)
	{
		std::cout << sensor_id_ << " lidar time sync failed, cur_ts[" << cur_ts << "], lidar ts[" << time_stamp << "]\n";
		ec_add("1022011", "lidar time sync failed", "input", 3);
	}
	else
	{
		ec_remove("1022011", 3);
	...
	if (pkt->data_header.FrameID() != frame_id_) //new frame
	{
		if (decoder_config.decoder_debug)
		{
			std::cout << "\n/***********************************/\nreceive new frame, FrameID: " <<  pkt->data_header.FrameID() << "\n";
		}
		if (-1 == frame_id_) //first frame
		{
			...
			if (0 == pkt->data_header.PkgPosition()) //first frame, first packet
			{
				if (decoder_config.decoder_debug)
				{
					std::cout << "first frame , first packet is head packet" << "\n";
				}
				pub_logic = 2;
			}
			else if (1 == pkt->data_header.PkgPosition())
			{
				if (decoder_config.decoder_debug)
				{
					std::cout << "first frame , first packet is middle packet" << "\n";
				}
				pub_logic = 2;
			}
			else if (2 == pkt->data_header.PkgPosition())
			{
				if (decoder_config.decoder_debug)
				{
					std::cout << "first frame , first packet is end packet, drop it" << "\n";
				}
				pub_logic = 0;
				return;
			}
			is_new = true;
			udp_pkg_count_ = 0;
			first_pkg_ts = time_stamp; //double, second
			frame_id_ = pkt->data_header.FrameID() % 65536;
			udp_pkg_id_ = pkt->data_header.PackageSeqNum();
		}
		else //not first frame
		{
			if ((((frame_id_ + 1) % 65536) != pkt->data_header.FrameID()) && (-1 != frame_id_))
			{
				std::cout << "lost frame, last:current -> " << frame_id_ << ":" <<  pkt->data_header.FrameID() << "\n";
				ec_add("1022007", "lost frame", "input");
			}
			else
			{
				ec_remove("1022007");
			...
			{
				if (0 == pkt->data_header.PkgPosition()) //new frame, first packet
				{
					if (decoder_config.decoder_debug)
					{
						std::cout << "finish last frame process, receive new frame , first packet is head packet" << "\n";
					}
					pub_logic = 2;
				}
				else if (1 == pkt->data_header.PkgPosition())
				{
					if (decoder_config.decoder_debug)
					{
						std::cout << "finish last frame process, receive new frame , first packet is middle packet" << "\n";
					}
					pub_logic = 2;
				}
				else if (2 == pkt->data_header.PkgPosition())
				{
					if (decoder_config.decoder_debug)
					{
						std::cout << "finish last frame process, receive new frame , first packet is end packet, drop it" << "\n";
					}
					pub_logic = 0;
					return;
				}
			...
			else //not finish last frame process
			{
				...
				pub_logic = 1;
				udp_pkg_count_ = 0;
				first_pkg_ts = time_stamp; //only exceed at new frame
				frame_id_ = pkt->data_header.FrameID() % 65536;
				udp_pkg_id_ = pkt->data_header.PackageSeqNum();
			}
		}
	...
	{
		if ((2 + udp_pkg_id_) != pkt->data_header.PackageSeqNum())
		{
			ec_add("1022006", "frame %d lost packet", "input", 10);
			if (decoder_config.error_debug & 8)
				printf("frame %d lost packet, last %d, current %d ts %.3f\n", pkt->data_header.FrameID(), udp_pkg_id_, pkt->data_header.PackageSeqNum(), time_stamp);
		}
		else
		{
			ec_remove("1022006", 10);
		}
		udp_pkg_count_ ++;
		udp_pkg_id_ = pkt->data_header.PackageSeqNum();

		if (is_new && (1 == pkt->data_header.PkgPosition())) //0:头帧;1:中间帧;2:尾帧
		{
			pub_logic = 2;
		}
		else if (is_new && (2 == pkt->data_header.PkgPosition())) {
			if (decoder_config.decoder_debug)
			{
				std::cout << "receive end packet\n";
			}
			if (pkt->data_header.PackageSeqNum() != 478) // one frame 240 pkts, there is two DataBlock in one pkt
			{
				std::cout << "end PkgID should be 478, but real id is: " << udp_pkg_id_ << "\n";
			}
			pub_logic = 3;
			is_new = false;
			udp_pkg_count_ = -1;
	...
	if (decoder_config.decoder_debug)
	{
		printf("   DataBlockNum in udp bag is: %d, FrameID %d, PkgID %d, PkgCnt %d, PkgPosition %d, ts %.4f\n", pkt->data_header.get_DataBlockNum(),
		       pkt->data_header.FrameID(),	pkt->data_header.PackageSeqNum(), udp_pkg_count_, pkt->data_header.PkgPosition(), time_stamp);
	}
	...
	for (int j = 0; j < pkt->data_header.get_DataBlockNum(); j++) // 2 DataBlock
	{
		...
		if (Column_ID >= 480 || LUTIndex > 3 ) {
			...
			ec_add("1022005", "error Column_ID || LUTIndex", "parse", 3);
			return;
		}
		else
		{
		...
		for (int i = 0; i < MAX_LIDAR_RING; i++) {	// 96 channel
			...
			point.latency = (time_stamp-first_pkg_ts)*1e9; //nanosecond
...
void HWDecoder::ConvertHuaweiPacktsToPoint(const ara::adsfi::CommonUdpPacket &packet, ara::adsfi::MsgLidarFrame &point_cloud) {
	HWPointCloudProtocol *pkt = (HWPointCloudProtocol *)(&(packet.udp_packet[0]));
	if (decoder_config.print_data)
	{
		std::stringstream its_message;
	...
	// avos::std_msg::Time pb_time_2;
	// pb_time_2.set_sec(avos::common::NodeTime(time_stamp).GetSec());//time_stamp is float s.
	// pb_time_2.set_nsec(avos::common::NodeTime(time_stamp).GetNsec());
	// pcl_conversions::toPCL(pb_time_2, point_cloud.pointCloud.header.stamp); //headr.stamp is ns
	...
	for (int j = 0; j < pkt->data_header.get_DataBlockNum(); j++) // 2 DataBlock
	{
		...
		for (int i = 0; i < MAX_LIDAR_RING; i++) {	// 192 data blocks
...
bool HWDecoder::isIpReachable(const std::string& ip, int debug) {
    ...
    if(net_card != "")
    {
        cmd = "ping -I " + net_card + " -c 1 -W 1 192.168.1.201 " + " > /dev/null 2>&1";
    }
    else
    {
        cmd = "ping -c 1 -W 1 192.168.1.201  > /dev/null 2>&1";
    }
	if(debug)
