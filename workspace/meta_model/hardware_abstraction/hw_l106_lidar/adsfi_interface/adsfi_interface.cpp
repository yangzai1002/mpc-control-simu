...
void AdsfiInterface::Init(){
    ...
    ApInfo("init") << "driver_ptr->driver_cfg.udp_buf_size = " << driver_ptr->driver_cfg.udp_buf_size;
    ApInfo("init") << "sensor_id = " << driver_ptr->sensor_id_;
    // ApInfo("init") << "driver_ptr->driver_cfg.lidar_type = " << driver_ptr->driver_cfg.lidar_type <<"\n";
    ApInfo("init") << "phase_lock(degree x 100): " << driver_ptr->driver_cfg.phase_lock;
    ApInfo("init") << "driver_ptr->driver_cfg.native_ip = " << driver_ptr->driver_cfg.native_ip;
    // ApInfo("init") << "driver_ptr->driver_cfg.pointcloud_port = " << driver_ptr->driver_cfg.pointcloud_port;
    // ApInfo("init") << "driver_ptr->driver_cfg.checkalive_port = " << driver_ptr->driver_cfg.checkalive_port;
    // ApInfo("init") << "driver_ptr->driver_cfg.pub_pointcloud = " << driver_ptr->driver_cfg.pub_pointcloud;
    ApInfo("init") << "driver_ptr->driver_cfg.ten_ms_filter_l = " << driver_ptr->driver_cfg.ten_ms_filter_l;
    ApInfo("init") << "driver_ptr->driver_cfg.ten_ms_filter_r = " << driver_ptr->driver_cfg.ten_ms_filter_r;
    ApInfo("init") << "debug_mask = " << debug_mask;
    ApInfo("init") << "driver_ptr->lidar_cali_config.print_data = " << driver_ptr->lidar_cali_config.print_data;
    ApInfo("init") << "driver_ptr->lidar_cali_config.decoder_debug = " << driver_ptr->lidar_cali_config.decoder_debug
             ;
    ApInfo("init") << "driver_ptr->lidar_cali_config.error_debug = " << driver_ptr->lidar_cali_config.error_debug
             ;
    ApInfo("init") << "driver_ptr->driver_cfg.interval_ms = " << driver_ptr->driver_cfg.interval_ms;
    ApInfo("init") << "driver_ptr->driver_cfg.debug_ts = " << driver_ptr->driver_cfg.debug_ts;
    ApInfo("init") << "driver_ptr->driver_cfg.debug = " << driver_ptr->driver_cfg.debug;
    // ApInfo("init") << "UDP_BUFFER_SIZE = " << UDP_BUFFER_SIZE;
    ...
    ApInfo("init") << "driver_ptr->difop_config.native_port = " << driver_ptr->difop_config.native_port;
    ApInfo("init") << "driver_ptr->msop_config.native_port = " << driver_ptr->msop_config.native_port;
...
void AdsfiInterface::ReadLidarConfig(const std::string calibration_key, const std::string &vehicle_param_key,
    ...
    if (ptr->GetProjectConfigArray(lidar_name + "/lidar_t", lidar_t)) {
        ...
        driver->ec_add("1022002", "Failed to read lidar_t parameter of "+lidar_name, "init");
        // exit(1);
    ...
    if (ptr->GetProjectConfigArray(lidar_name + "/lidar_R", lidar_R)) {
        ...
        driver->ec_add("1022002", "Failed to read lidar_R parameter of "+lidar_name, "init");
        // printf("Failed to read lidar_t parameter of %s\n", lidar_name.c_str());
