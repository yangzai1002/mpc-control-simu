...
public:
    ...
    void Init(const std::string &endpoint, const std::string &topic, int32_t timeout) {
        ...
        try {
            ApInfo("vehicle_chassis_recv") << "初始化ZMQ订阅: endpoint=" << endpoint << ", topic=" << topic;
            ...
            // 异常处理
            ApInfo("vehicle_chassis_recv") << "初始化ZMQ订阅失败: " << e.what();
    ...
    void GetVehicleInformation(ara::adsfi::VehicleInformation &vehicle_info) {
        ...
        if (timeout) {
            ...
            ApInfo("vehicle_chassis_recv") << "数据读取超时，已上报故障码" << _ERRORCODE_DATA_FREQUENCY_ERROR;
        } else {
    ...
    void RecvLoop() {
        ...
        while (true) {
            ...
            try {
                ...
                ApInfo("vehicle_chassis_recv") << "[ZMQ] 接收到数据，长度: " << data_msg.size();

                const uint8_t *buffer = (const uint8_t *)data_msg.data();
                size_t         length = data_msg.size();

                ApInfo("vehicle_chassis_recv") << "[ZMQ] 收到数据的长度: " << length;
                if (length != kFullLen) {
                    ApError("vehicle_chassis_recv")
                        << "[ZMQ] 收到包长度 " << length << "，期望 " << kFullLen << "，丢弃该包";
                    _fault.add(_ERRORCODE_DATA_READ_ERROR, "数据读取失败");
                    ApInfo("vehicle_chassis_recv") << "数据读取失败，已上报故障码" << _ERRORCODE_DATA_READ_ERROR;
                    continue;
                }
                _fault.remove(_ERRORCODE_DATA_READ_ERROR);

                // 打印接收数据长度和全部 HEX
                ApInfo("vehicle_chassis_recv") << "[ZMQ] 接收到数据长度: " << length << " 字节";
                if (kPrintFullPayload) {
                    ...
                    ApInfo("vehicle_chassis_recv") << oss_all.str();
                }
                ...
                try {
                    ...
                    ApError("vehicle_chassis_recv") << "[ZMQ][ParseError] " << e.what() << " 包中 checksum=0x"
                                                    << std::hex << pkg_checksum << std::dec;
                    ...
                    ApError("vehicle_chassis_recv") << oss_err.str();
                ...
                info.header.time.nsec =
                    static_cast<decltype(info.header.time.nsec)>((double_time - info.header.time.sec) * 1e9);
                ...
                if (vcu > 0) {
                    ...
                    ApInfo("vehicle_chassis_recv")
                        << "[VCU] 级别=" << vcu << " 描述=" << desc << " 已上报故障码=" << _ERRORCODE_CHASSIS_ERROR;
                }
                ...
                ApInfo("vehicle_chassis_recv") << "[ZMQ] RecvLoop 异常: " << e.what();
...
//                 ApInfo("vehicle_chassis_recv") << "初始化参数加载错误: read local config config_path_prefix failed,
//                 上报故障码: "
...
//             }
//         } else {
//             ApInfo("vehicle_chassis_recv") << "获取CustomStack实例失败";
//         }
//     }
