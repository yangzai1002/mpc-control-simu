...
#pragma pack(push, 1)
    ...
    struct CommunicationHead {
        ...
        uint32_t message_id = 0x00;

        // 消息帧长度
        // 消息帧长度（不含）之后的全部字节长数
        uint32_t message_length = 0x00;

        // 客户端ID
        // 由通信系统确定
        uint16_t client_id = 0x00;

        // 会话ID
        // 区分同一客户端请求同一服务Service的次数，默认为0
        uint16_t session_id = 0x00;
        ...
        // 接口版本
        // 默认为0x01
        uint8_t interface_version = 0x00;
    ...
    struct InternalFrameHeader {
        ...
        // Byte3：（0-23）时，Byte4：（0-59）分，Byte5：（0-59）秒，Byte6和Byte7：（0-999）毫秒
