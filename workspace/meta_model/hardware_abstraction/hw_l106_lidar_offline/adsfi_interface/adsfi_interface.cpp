...
void AdsfiInterface::Init(){
    ...
    lidar_packets_ptr = nullptr;

    auto ptr = CustomStack::Instance();
    driver_ptr = std::make_shared<L106DriverOffline>();
    ...
    driver_ptr->lidar_cali_config.error_debug = (debug_mask & 8);
    ...
    driver_ptr->lidar_name_ = lidar_name;

 
    ReadLidarConfig(calibration_key, vehicle_param_key, lidar_name, driver_ptr->lidar_cali_config, driver_ptr.get());
    driver_ptr->decoder.Init(driver_ptr->lidar_cali_config);
...
int AdsfiInterface::Process(const std::string name,std::shared_ptr<ara::adsfi::MsgLidarFrame> &msg){
	ara::adsfi::SensorLidarPacket pakcets;
    GetLidarPackets(pakcets);
    driver_ptr->DecodePackets(pakcets, *msg);
    return 0;
}



void AdsfiInterface::Callback(const std::string name,const std::shared_ptr<ara::adsfi::SensorLidarPacket> &msg){
	SetLidarPackets(msg);
