...
bool PlannerManager::RunOnce(const std::vector<ref_line::RawPoint>& raw_ref_points,
    ...
    double start_dl = current_path_state[2];
    if (!last_trajectory_.empty() && last_trajectory_.size() > 2) {
        double ds = last_trajectory_[1].s - last_trajectory_[0].s;
        if (ds > 0.01) {
            start_dl = (last_trajectory_[1].l - last_trajectory_[0].l) / ds;
        }
    }
    ...
    while (em_iter < 2 && !em_converged) {  // 最多迭代 2 次
        path_planner::PathOptimizerQP::Config path_cfg;
        path_cfg.weight_ddl = dynamic_weight_ddl;
        path_planner::PathOptimizerQP qp_path_opt(path_cfg);
        ...
        dp_decider_->Execute(obstacles_sl,
                             {current_path_state[0], current_path_state[1], start_dl},
                             current_v,
                             last_trajectory_,
                             dp_path);
        ...
        if (!path_success) {
            ...
            if (path_success) {
                active_speed_limit = 10.0 / 3.6;  // ✅ 修复：10km/h
            } else {
                std::cerr << "[Manager] ❌ Path QP Failed even in survival mode!" << std::endl;
                break; 
            }
        ...
        //s_gen_->Generate(smooth_ref, final_path_sl, ref_speed_traj, active_speed_limit,last_plan_vel,current_speed_state[2]); 
        s_gen_->Generate(smooth_ref, final_path_sl, ref_speed_traj, active_speed_limit); 
    ...
    if (success) {
        ...
        std::cerr << "[Manager] Speed QP Failed! Outputting internal emergency trajectory." << std::endl;
        final_trajectory = ref_speed_traj; // 降级：直接输出速度生成器结果
        last_trajectory_ = final_trajectory; 
    }
