...
int main() {
    std::cout << "[INFO] Booting Interactive Single-Frame Snapshot Tester..." << std::endl;
    PlannerInterface planner_interface;

    const double START_UTM_X = 500000.0;
    const double START_UTM_Y = 3450000.0;

    double ego_x = START_UTM_X;
    double ego_y = START_UTM_Y;
    double ego_theta = std::atan2(10.0, 2.0); 
    double ego_v = 8.5;  
    double ego_a = 0.0;  
    ...
    Location sensor_loc;
    SimUTMToLatLon(ego_x, ego_y, sensor_loc.latitude, sensor_loc.longitude);
    sensor_loc.height = 0.0;
    sensor_loc.veloity = ego_v;
    sensor_loc.acc = ego_a;
    double heading_deg = 90.0 - (ego_theta * 180.0 / M_PI);
    ...
    for (const auto& obs : sim_obstacles) {
        double rel_x = obs.x - ego_x;
        double rel_y = obs.y - ego_y;
    ...
    bool success = planner_interface.RunPlanner(sensor_loc, interface_global_path, occupancy_grid, 5.0, output_trajectory);

    double min_obs_dist = 999.0;
    for(const auto& obs : sim_obstacles) {
        double dist = std::hypot(obs.x - ego_x, obs.y - ego_y);
        if(dist < min_obs_dist) min_obs_dist = dist;
    }

    // 支持窗口放缩配置
    cv::namedWindow("Autonomous Advanced Testbed", cv::WINDOW_NORMAL);
    cv::resizeWindow("Autonomous Advanced Testbed", CANVAS_W + PANEL_W, CANVAS_H);

    double current_scale = 10.0; // 初始缩放比例

    // 渲染循环 (允许通过按键交互调整视角和缩放，但不再进行动力学积分)
    while (true) {
        ...
        auto to_px = [&](double x, double y) {
            return cv::Point(view_center.x + (x - ego_x) * current_scale, view_center.y - (y - ego_y) * current_scale);
        };

        // 绘制物理网格线
        for (int i = 0; i <= CANVAS_W; i += static_cast<int>(10.0 * current_scale)) {
            int offset_x = i - static_cast<int>(std::fmod(ego_x * current_scale, 10.0 * current_scale));
            int offset_y = i - static_cast<int>(std::fmod(ego_y * current_scale, 10.0 * current_scale));
        ...
        DrawVehicleHUD(canvas, view_center, ego_theta, current_scale);
        ...
        cv::putText(panel, "INTERACTIVE SNAPSHOT UI", {30, text_y}, cv::FONT_HERSHEY_SIMPLEX, 0.7, {255, 255, 255}, 2, cv::LINE_AA);
        text_y += 20;

        draw_section_title("[ EGO KINEMATICS ]");
        snprintf(buf, sizeof(buf), "%.2f km/h", ego_v * 3.6); draw_kv("Current Speed:", buf, {0, 255, 0});
        snprintf(buf, sizeof(buf), "%.2f m/s^2", ego_a);      draw_kv("Acceleration:", buf);
        snprintf(buf, sizeof(buf), "%.1f deg", heading_deg);  draw_kv("Math Heading:", buf);
        snprintf(buf, sizeof(buf), "%.1f, %.1f", ego_x, ego_y); draw_kv("UTM (X,Y):", buf);
        ...
        text_y += 25;
        cv::putText(panel, "=== INTERACTIVE CONTROLS ===", {20, text_sensor_locy}, cv::FONT_HERSHEY_SIMPLEX, 0.6, {0, 255, 255}, 1, cv::LINE_AA); text_y += 25;
        ...
        cv::putText(panel, "[ ESC ]       : Exit Program", {20, text_y}, cv::FONT_HERSHEY_SIMPLEX, 0.5, {0, 0, 255}, 1, cv::LINE_AA);

        cv::imshow("Autonomous Advanced Testbed", display);
        
        // 键盘交互响应
        int key = cv::waitKey(50);
        if (key == 27) break; // ESC 退出
        if (key == '=' || key == '+') current_scale *= 1.1; // 放大
        if (key == '-' || key == '_') current_scale /= 1.1; // 缩小
    }
