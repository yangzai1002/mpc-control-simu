...
void convertEulerAngleToMonotonic(std::vector<double> &a)
{
  for (unsigned int i = 1; i < a.size(); ++i)
  {
    ...
    a[i] = amathutils::normalizeRadian(a[i]); 
  }
}

/**
 * @brief 从 Roll/Pitch/Yaw 计算四元数（Z-Y-X 旋转顺序）
 * @param roll 滚转角（弧度，绕X轴）
 * @param pitch 俯仰角（弧度，绕Y轴）
 * @param yaw 偏航角（弧度，绕Z轴）
 * @return tf2::Quaternion 四元数（tf2库类型，可直接用于之前的控制逻辑）
 */
...
void calcTrajectoryYawFromXY(MPCTrajectory &traj)
{
  if (traj.yaw.size() == 0)
    return;
  int current_dir = 0;
  if(traj.vx.at(0) <0.0)/*如果速度为负值，则反向计算yaw*/
  {
    current_dir = -1;
  }else
  {
...
//  for (const auto &wp : pathTrajectory.trajectoryPoints)
//  {
//    const double x = wp.Waypoint.x;
//    const double y = wp.Waypoint.y;
//    const double z = wp.Waypoint.z;
...
void convertWaypointsToMPCTrajWithDistanceResample(const HafEgoTrajectory &pathTrajectory, const std::vector<double> &path_time,
                                                             const double &dl, MPCTrajectory &ref_traj)
{
  ref_traj.clear();
  double dist = 0.0;
  ...
  for (int i = 1; i < (int)path_time.size(); ++i)
  {
    double dx = pathTrajectory.trajectoryPoints.at(i).wayPoint.x - pathTrajectory.trajectoryPoints.at(i-1).wayPoint.x;
    double dy = pathTrajectory.trajectoryPoints.at(i).wayPoint.y - pathTrajectory.trajectoryPoints.at(i-1).wayPoint.y;
    dist += sqrt(dx * dx + dy * dy);
    dists.push_back(dist);
  }

  convertWaypointsToMPCTrajWithResample(pathTrajectory, path_time, dists, dl, ref_traj);
}
...
//    const double yaw0 = path.waypoints.at(j - 1).yaw;
//    const double yawx = path.waypoints.at(j).yaw;
//    //const double dyaw = amathutils::normalizeRadian(tf2::getYaw(pos1.orientation) - yaw0);
//    const double dyaw = amathutils::normalizeRadian(yawx - yaw0);
...
//    if(isnan(x)||isnan(y))/*判断一下是不是有无穷大*/
//    {
//       point += d_ref_index;
//      continue;
//    }
...
//  }
//}
void convertWaypointsToMPCTrajWithResample(const HafEgoTrajectory &pathTrajectory, const std::vector<double> &path_time,
                                                     const std::vector<double> &ref_index, const double &d_ref_index, MPCTrajectory &ref_traj)
{
  if (ref_index.size() == 0) {
    return;
...
//    const geometry_msgs::Twist twist0 = path.waypoints.at(j - 1).twist.twist;
//    const geometry_msgs::Twist twist1 = path.waypoints.at(j).twist.twist;
    const double x = ((ref_index_dist - a) * pathTrajectory.trajectoryPoints.at(j-1).wayPoint.x + a * pathTrajectory.trajectoryPoints.at(j).wayPoint.x) / ref_index_dist;
    const double y = ((ref_index_dist - a) * pathTrajectory.trajectoryPoints.at(j-1).wayPoint.y + a * pathTrajectory.trajectoryPoints.at(j).wayPoint.y) / ref_index_dist;
    const double z = ((ref_index_dist - a) * pathTrajectory.trajectoryPoints.at(j-1).wayPoint.z + a * pathTrajectory.trajectoryPoints.at(j).wayPoint.z) / ref_index_dist;

    /* for singular point of euler angle */
    //const double yaw0 = tf2::getYaw(pos0.orientation);
//    const double yaw0 = path.waypoints.at(j - 1).yaw;
//    const double yawx = path.waypoints.at(j).yaw;
    const double yaw0 = pathTrajectory.trajectoryPoints.at(j-1).wayPoint.theta;
    const double yawx = pathTrajectory.trajectoryPoints.at(j).wayPoint.theta;
    //const double dyaw = amathutils::normalizeRadian(tf2::getYaw(pos1.orientation) - yaw0);
    const double dyaw = amathutils::normalizeRadian(yawx - yaw0);
    const double yaw1 = yaw0 + dyaw;
    const double yaw = ((ref_index_dist - a) * yaw0 + a * yaw1) / ref_index_dist;
   // const double vx = ((ref_index_dist - a) * twist0.linear.x + a * twist1.linear.x) / ref_index_dist;
    const double vx = ((ref_index_dist - a) * pathTrajectory.trajectoryPoints.at(j-1).speed + a * pathTrajectory.trajectoryPoints.at(j).speed) / ref_index_dist;
    const double curvature_tmp = 0.0;
    const double t = ((ref_index_dist - a) * path_time.at(j - 1) + a * path_time.at(j)) / ref_index_dist;
    if(std::isnan(x)||std::isnan(y))/*判断一下是不是有无穷大*/
    {
       point += d_ref_index;
      continue;
    } 
...
//  }
//}
void calcPathRelativeTime(const HafEgoTrajectory &pathTrajectory, std::vector<double> &path_time)
{
  double t = 0.0;
  path_time.clear();
  path_time.push_back(t);
  for (int i = 0; i < (int)pathTrajectory.trajectoryPoints.size() - 1; ++i)
  {
    const double x0 = pathTrajectory.trajectoryPoints.at(i).wayPoint.x;
    const double y0 = pathTrajectory.trajectoryPoints.at(i).wayPoint.y;
    const double z0 = pathTrajectory.trajectoryPoints.at(i).wayPoint.z;
    const double x1 = pathTrajectory.trajectoryPoints.at(i+1).wayPoint.x;
    const double y1 = pathTrajectory.trajectoryPoints.at(i+1).wayPoint.y;
    const double z1 = pathTrajectory.trajectoryPoints.at(i+1).wayPoint.z;
    const double dx = x1 - x0;
    const double dy = y1 - y0;
    const double dz = z1 - z0;
    const double dist = sqrt(dx * dx + dy * dy + dz * dz);
    double v = std::max(std::fabs(pathTrajectory.trajectoryPoints.at(i).speed), 1.0);
    t += (dist / v);
    path_time.push_back(t);
  }
}

bool calcNearestPose(const MPCTrajectory &traj,const Adsfi::HafPose &self_pose, Adsfi::HafPose &nearest_pose,
                               unsigned int &nearest_index, double &min_dist_error, double &nearest_yaw_error, double &nearest_time)
{
  ...
  for (uint i = 0; i < traj.size(); ++i)
  {
    ...
    const double err_yaw = amathutils::normalizeRadian(getYaw(self_pose.orientation) - traj.yaw[i]);
...
bool calcNearestPoseInterp(const MPCTrajectory &traj, const Adsfi::HafPose &self_pose, Adsfi::HafPose &nearest_pose,
                                     unsigned int &nearest_index, double &min_dist_error, double &nearest_yaw_error, 
                                     double &nearest_time,double &max_yaw_error)
{
  ...
  //const double my_yaw = getYaw(self_pose.orientation);
  double my_yaw = self_pose.orientation.z;
  int nearest_index_tmp = -1;
  double min_dist_squared = std::numeric_limits<double>::max();
  for (uint i = 0; i < traj.size(); ++i)
  {
  ...
  if (nearest_index_tmp == -1)
  {
    ...
    printf("[calcNearestPoseInterp] yaw error is over %f deg for all waypoints. no closest waypoint found.",amathutils::rad2deg(max_yaw_error));
    return false;
  }
  nearest_index = nearest_index_tmp;
  if (traj.size() == 1)
  {
    ...
    nearest_pose.position.y = traj.y[nearest_index];
    Adsfi::HafQuaternion q;
    q = eulerToQuaternion(0, 0, traj.yaw[nearest_index]);
    nearest_pose.orientation = q;
    nearest_time = traj.relative_time[nearest_index];
    min_dist_error = std::sqrt(min_dist_squared);
  ...
  double tmp_yaw_err = amathutils::normalizeRadian(traj.yaw[nearest_index] - traj.yaw[second_nearest_index]);
  if (tmp_yaw_err > M_PI)
  {
  ...
  const double nearest_yaw = amathutils::normalizeRadian(traj.yaw[second_nearest_index] + alpha * tmp_yaw_err);
  // ROS_WARN("nearest_yaw:%f,traj.yaw[second_nearest_index]:%f,alpha :%f,tmp_yaw_err:%f\n",nearest_yaw,
