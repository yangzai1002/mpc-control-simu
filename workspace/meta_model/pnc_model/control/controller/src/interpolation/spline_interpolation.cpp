...
inline std::vector<double> solveTridiagonalMatrixAlgorithm(
        const TDMACoef &tdma_coef)
{
                                  ...
                                  const std::vector<double> &query_keys)
{
    ...
    {
        const double m_val = (base_values.at(i + 1) - base_values.at(i))
    ...
    for (size_t i = 0; i < base_keys.size(); ++i)
    {
        ...
        const double s_val = (std::abs(m_values.at(i + 1) - m_values.at(i))
                * m_values.at(i - 1)
                + std::abs(m_values.at(i - 1) - m_values.at(i - 2))
                        * m_values.at(i)) / denom;
        s_values.push_back(s_val);
    ...
    for (size_t i = 0; i < base_keys.size() - 1; ++i)
    {
        ...
        b.push_back(
                (3.0 * m_values.at(i) - 2.0 * s_values.at(i)
                        - s_values.at(i + 1))
                        / (base_keys.at(i + 1) - base_keys.at(i)));
        ...
        const std::vector<double> &base_values)
{
    ...
    if (num_base > 2)
    {
        ...
        for (size_t i = 0; i < num_base - 2; ++i)
        {
            ...
            tdma_coef.d[i] = 6.0
                    * (diff_values[i + 1] / diff_keys[i + 1]
                            - diff_values[i] / diff_keys[i]);
        }

        const std::vector<double> tdma_res = solveTridiagonalMatrixAlgorithm(
                tdma_coef);
    ...
    multi_spline_coef_ = interpolation::MultiSplineCoef
    { num_base - 1 };    // N
    for (size_t i = 0; i < num_base - 1; ++i)
    {
        multi_spline_coef_.a[i] = (v[i + 1] - v[i]) / 6.0 / diff_keys[i];
        multi_spline_coef_.b[i] = v[i] / 2.0;
        multi_spline_coef_.c[i] = diff_values[i] / diff_keys[i]
                - diff_keys[i] * (2 * v[i] + v[i + 1]) / 6.0;
