...
PIDController::PIDController() :
        m_error_integral(0.0), m_prev_error(0.0), m_is_first_time(true), m_is_gains_set(
                false), m_is_limits_set(false)
{
}


double PIDController::calculate(const double error, const double dt,
        const bool enable_integration, std::vector<double> &pid_contributions)
{
    ...
    if (enable_integration)
    {
        ...
        m_error_integral = std::min(
                std::max(m_error_integral, p.min_ret_i / p.ki),
                p.max_ret_i / p.ki);
    }
...
//
void PIDController::setLimits(const double max_ret, const double min_ret,
        const double max_ret_p, const double min_ret_p, const double max_ret_i,
        const double min_ret_i, const double max_ret_d, const double min_ret_d)
{
