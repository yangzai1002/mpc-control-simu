...
// vpresults_ptr->header.timestamp.sec = ADSFI_GET_SEC(time_sec);
// vpresults_ptr->header.timestamp.nsec = ADSFI_GET_NSEC(time_sec);
// vpresults_ptr->header.frameID = out_results.header.data_name;
// vpresults_ptr->header.time_chain_vec.resize(1);
// vpresults_ptr->header.seq = ++seq_;
// vpresults_ptr->header.time_chain_vec[0].t1 = out_results.receive_stamp;
// // vpresults_ptr->header.time_chain_vec.from_module = out_results.start_stamp;
// vpresults_ptr->header.time_chain_vec[0].t2 = out_results.send_stamp;
...
//     vpresults_ptr->vp_result_vec.push_back(cam_res);
//   }
// }
// }
...
int AdsfiInterface::Process(const std::string &name,std::shared_ptr<ara::adsfi::MsgCameraVPResult> &vpresults_ptr) {
    ...
    if(yolo_->GetData(detections, tm)){
        vpresults_ptr->header.timestamp.sec = tm.tv_sec;
        vpresults_ptr->header.timestamp.nsec =tm.tv_usec * 1e3;
        vpresults_ptr->header.frameID = "";
        vpresults_ptr->header.time_chain_vec.resize(1);
        vpresults_ptr->header.seq = ++seq_;
        vpresults_ptr->header.time_chain_vec[0].t1 = tm.tv_sec + tm.tv_usec * 1e-6;
        gettimeofday(&tm, NULL);
        vpresults_ptr->header.time_chain_vec[0].t2 =tm.tv_sec + tm.tv_usec * 1e-6;
        std::cout << "[CUSTOM INFO ] ############### INTERFACE GetData YOLO detection size "<<detections.size()<<"\n";
        
        ara::adsfi::MsgCameraDetectionResult cam_res;
        cam_res.header.frameID = "front_middle";
        cam_res.image_objects.detectionOut2d.reserve(detections.size());

        for(const auto& det:detections)
        {

            ara::adsfi::MsgHaf2dDetectionOut object2d;
            object2d.cls = det.classId;          // 类别
            object2d.confidence = det.confidence;      // 置信度
            object2d.objectId = det.track_id;
            // 若需保留center/size/location（可选）
            object2d.rect.size.x = det.width;
            object2d.rect.size.y = det.height;
            object2d.rect.center.x = det.centerx;
            object2d.rect.center.y = det.centery;
            cam_res.image_objects.detectionOut2d.push_back(object2d);
        }
