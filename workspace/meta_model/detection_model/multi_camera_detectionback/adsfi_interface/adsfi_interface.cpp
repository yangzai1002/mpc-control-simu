...
void AdsfiInterface::Init()  {
    ...
    avos::common::AvosNode::Init("MultiCameraDetection",true,"/opt/usr/zxz/log/","/opt/usr/zxz/log/","/opt/usr/zxz/log/");
    if(b_use_tcp11)
      m_socket_client.connect(SERVER_HOST,SERVER_PORT);
	}
  ...
  void AdsfiInterface::ConvertToMsgCameraVPResult(const PerceptionVPResult &out_results,ara::adsfi::MsgCameraVPResult &vpresults)
  {
    ...
    if(handle.init_status<=0)
    {
      ...
      if(temp_vpres->camera_vpresults.size() > 1 && b_use_tcp11) 
      {
        m_socket_client.send_all(temp_vpres->camera_vpresults[1].vision_segment.seg.data.data(),
                            temp_vpres->camera_vpresults[1].vision_segment.seg.data.size());
      }
