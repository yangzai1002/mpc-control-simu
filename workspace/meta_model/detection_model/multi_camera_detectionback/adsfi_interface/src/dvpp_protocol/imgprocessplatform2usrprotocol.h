...
enum EIdpMdc610ImgType
{
    ...
    IMG_FORMAT_S8C1 = 75,                 // Signed 8bit for 1pixel 1Channel，每个像素用1个8bit有符号数据表示的单通道图像，当前不支持该格式
    IMG_FORMAT_S8C2_PACKAGE = 76,         // Signed 8bit for 1pixel 2Channel Package，每个像素用2个8bit有符号数表示的双通道图像Package格式，当前不支持该格式
    IMG_FORMAT_S8C2_PLANAR = 77,          // Signed 8bit for 1pixel 2Channel Planar，每个像素用2个8bit有符号数据表的双通道图像Planar格式，当前不支持该格式
    IMG_FORMAT_S16C1 = 78,                // Signed 16bit 1pixel 1Channel，每个像素用1个16bit有符号数据表示的单通道图像，当前不支持该格式
    IMG_FORMAT_U8C1 = 79,                 // Unsigned 8bit 1pixel 1Channel，每个像素用1个8bit无符号数据表示的单通道图像，当前不支持该格式
    IMG_FORMAT_U16C1 = 80,                // Unsigned 16bit 1pixel 1Channel，每个像素用1个16bit无符号数据表示的单通道图像，当前不支持该格式
    IMG_FORMAT_S32C1 = 81,                // Signed 32bit 1pixel 1Channel，每个像素用1个32bit有符号数据表示的单通道图像，当前不支持该格式
    IMG_FORMAT_U32C1 = 82,                // Unsigned 32bit 1pixel 1Channel，每个像素用1个32bit无符号数据表示的单通道图像，当前不支持该格式
    IMG_FORMAT_U64C1 = 83,                // Unsigned 64bit 1pixel 1Channel，每个像素用1个64bit无符号数据表示的单通道图像，当前不支持该格式
    IMG_FORMAT_S64C1 = 84,                // Signed 64bit 1pixel 1Channel，每个像素用1个64bit有符号数据表示的单通道图像，当前不支持该格式
...
typedef struct SVpcCropResizePasteRegion {
    ...
    void SetResizeParam(u_int32_t origin_width, u_int32_t origin_height,
                        u_int32_t resize_width, u_int32_t resize_height, u_int32_t interpolation = 0) {
    ...
    void SetCropParam(u_int32_t crop_left_offset, u_int32_t crop_top_offset,
                      u_int32_t dest_width, u_int32_t dest_height
                     ) {
    ...
    void SetResizePaste(u_int32_t origin_width, u_int32_t origin_height,
                        u_int32_t resize_width, u_int32_t resize_height,
    ...
    void SetCropPaste(u_int32_t crop_width, u_int32_t crop_height,
                      u_int32_t crop_left_offset, u_int32_t crop_top_offset,
                      u_int32_t paste_left_offset, u_int32_t paste_top_offset
                     ) {
        crop_region.top_offset = crop_top_offset;
        crop_region.left_offset = crop_left_offset;
    ...
    void SetCropResizePaste(u_int32_t crop_width, u_int32_t crop_height,
                            u_int32_t crop_left_offset, u_int32_t crop_top_offset,
                            u_int32_t resize_width, u_int32_t resize_height,
                            u_int32_t paste_left_offset, u_int32_t paste_top_offset
                           ) {
...
struct SImgProcess
{
    ...
    void InitOutputImage(void *data, size_t _m_buf_len, u_int32_t _m_img_width, u_int32_t _m_img_height,
                         EIdpImgBufType _m_buf_memtype = BUF_DVPP_TYPE,
                         EIdpMdc610ImgType _m_buf_imgtype = IMG_FORMAT_YUV_SEMIPLANAR_420) {
        m_out_img.m_dest_pic_info.Init(data, _m_buf_len, _m_img_width, _m_img_height, _m_buf_memtype, _m_buf_imgtype);
    }
    ...
    void SetResizePaste(u_int32_t origin_width, u_int32_t origin_height,
        ...
        m_out_img.SetResizePaste(origin_width, origin_height, resize_width, resize_height, paste_left_offset, paste_top_offset );
    ...
    void SetCropPaste(u_int32_t crop_width, u_int32_t crop_height,
        ...
        m_out_img.SetCropPaste(crop_width, crop_height, crop_left_offset, crop_top_offset,   paste_left_offset, paste_top_offset  );
    ...
    void SetCropResizePaste(u_int32_t crop_width, u_int32_t crop_height,
        ...
        m_out_img.SetCropResizePaste(crop_width, crop_height, crop_left_offset, crop_top_offset,  resize_width, resize_height, paste_left_offset, paste_top_offset );
    }
