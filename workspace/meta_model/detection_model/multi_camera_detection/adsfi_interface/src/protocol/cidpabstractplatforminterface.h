...
public:
    ...
    int open(std::string so_name, int mode) {
        ...
        m_createinstance_func = (CreateIdpAbstractPlatformFunc)dlsym(
            handle, "CreateIdpAbstractPlatform");
        const char *error = dlerror();
        if (error) {
            std::cerr << "Error loading CreateIdpAbstractPlatform function: "
                      << error << std::endl;
            dlclose(handle);
            return -1;
        }

        m_destroyinstance_func = (DestroyIdpAbstractPlatformFunc)dlsym(
            handle, "DestroyIdpAbstractPlatform");
        error = dlerror();
        if (error) {
            std::cerr << "Error loading DestroyIdpAbstractPlatform function: "
                      << error << std::endl;
            dlclose(handle);
            return -1;
        }
        ...
        if (m_hal_platform->open() != 0) {
            std::cerr << "Failed to initialize m_hal_platform object"
                      << std::endl;
