...
namespace app_common {
    ...
    public:
        PathPlannerGlobalRawPlan(const std::string& osm_file, const std::string& roads_info) : PathPlanner() {
            ...
            ApInfo("task_executor") << "waypoints size: " << _readout_waypoints.size();
            if (_readout_waypoints.size() < _min_way_points) {
                ApError("task_executor") << "waypoints " << osm_file << " less than " << _min_way_points << ": "
        ...
        std::vector<std::vectorara::adsfi::usinessPoint> > paths() override {
