...
public:
  ...
  static int GeoTransform(const GeoPointGCCS &car_pgccs, const GeoPointGCCS &target_pgccs,
                          GeoPointVCS &output_pvcs) {
    ...
    if (output_pvcs.angle >= 360) output_pvcs.angle -= 360;
    return 0;
  }

  static int GeoTransform(const GeoPointGCCS &car_pgccs, const GeoPointVCS &target_pvcs,
    ...
    if (output_pgccs.angle >= 360) output_pgccs.angle -= 360;
    return 0;
  }
  ...
  static int GCCS2VCS(const GeoPointGCCS &car_pgccs, const GeoPointGCCS &target_pgccs,
                      GeoPointVCS &output_pvcs) {
    return GeoTransform(car_pgccs, target_pgccs, output_pvcs);
  }
  static int VCS2GCCS(const GeoPointGCCS &car_pgccs, const GeoPointVCS &target_pvcs,
                      GeoPointGCCS &output_pgccs) {
