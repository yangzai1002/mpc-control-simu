...
#define KEY_MSG_SIZE 52
    ...
    template <typename T> string vecPrintf(vector<T> &vec_data) {
        string        ret_string;
    ...
    void Print(char *data_pool_base, const std::string &prefix = "") {
        AINFO << getString(data_pool_base, prefix);
    }
...
#define DATA_POOL_AREA_SIZE 128
    ...
    void Print(char *data_pool_base, const std::string &prefix = "") {
        AINFO << prefix << "module_name:" << GetModuleName();
        AINFO << prefix << "data_pool_area_size:" << data_pool_area_size;
        for (int i = 0; i < data_pool_area_size; i++) {
...
#define MODULE_AREA_SIZE 512
    ...
    void Print(const std::string &prefix = "") {
        AINFO << prefix << "module_pool_size:" << module_pool_size;
    ...
    char *GetDataPoolPtr(const std::string &module_name, const std::string &key) {
        for (int i = 0; i < module_pool_size; i++) {
            if (param[i].GetModuleName() == module_name) {
                ...
                // AINFO << "offset " <<offset<<std::endl;
                if (offset == -1) {
                    AINFO << "NULL param[i].GetModuleName() " << param[i].GetModuleName() << " module_name "
                          << module_name << std::endl;
                    return NULL;
                }
                // AINFO << "param[i].GetModuleName() " <<param[i].GetModuleName() <<" module_name
                // "<<module_name<<std::endl;
...
public:
    ~SHMParamApi() {
        ...
        if (avos::common::SharedMemory::sharedMemoryInfo(current_shmid, info) < 0) {
            AINFO << "cannot get shmid info " << current_shmid;
            exit(0);
        }
...
public:
    ...
    bool QueryDoubleModuleParam(const std::string &key, double &value, const std::string module = "global") {
        char *ptr = shm_param_api_->GetDataPoolPtr(module, key);
        if (NULL == ptr) {
            AINFO << "not found key: " << key << std::endl;
            return false;
        } else {
            UDataShmAttribute attr = shm_param_api_->GetDataPoolAttr(module, key);
            if (attr.IData[0] == 0) {
                AINFO << "not found attr: " << key << std::endl;
                return false;
            }
    ...
    char *WriteNewDataPool(const string &value) {
        // AINFO << "para tools set value.size() = " << value.size() ;
    ...
    void WriteExistDataPool(const string &value, char *ptr) {
        // AINFO << "para tools set "<< " value = " << value ;
    ...
    bool SetStringModuleParam(const std::string &key, const std::string &value, const std::string module = "global") {
        ...
        // AINFO << "para tools set key = " << key << " value = " << value;
    ...
    char *WriteNewDataPool(const vector<double> &value) {
        ...
        memcpy(data_pool_ptr_->_data_pool_ptr_ + data_pool_ptr_->cur_data_pool_size, (char *)&value[0],
               value.size() * sizeof(double));
        data_pool_ptr_->cur_data_pool_size += DATA_POOL_DOUBLE_VEC_SIZE * sizeof(double);
        return data_pool_ptr_->_data_pool_ptr_ + data_pool_ptr_->cur_data_pool_size -
               DATA_POOL_DOUBLE_VEC_SIZE * sizeof(double);
    }
    void WriteExistDataPool(const vector<double> &value, char *ptr) {
        memcpy(ptr, (char *)&value[0], value.size() * sizeof(double));
    }

    bool SetDoubleModuleParam(const std::string &key, const vector<double> &value,
                              const std::string module = "global") {
        lock_data_pool();
        char *ptr = shm_param_api_->GetDataPoolPtr(module, key);
        ...
        attr.SData.isVecDouble_size =
            value.size() < DATA_POOL_DOUBLE_VEC_SIZE ? value.size() : DATA_POOL_DOUBLE_VEC_SIZE;
    ...
    char *WriteNewDataPool(const vector<int> &value) {
        ...
        return data_pool_ptr_->_data_pool_ptr_ + data_pool_ptr_->cur_data_pool_size -
               DATA_POOL_INT_VEC_SIZE * sizeof(int);
    }
    ...
    char *WriteNewDataPool(const vector<string> &value) {
        ...
        if (data_pool_ptr_->cur_data_pool_size + size > DATA_POOL_BUFSIZE || size > DATA_POOL_STRING_VEC_BUFSIZE) {
            AINFO << "this vector<string> is bigger than 30*1024 is " << size << std::endl;
            return NULL;
        }
    ...
    bool SetStringModuleParam(const std::string &key, const vector<string> &value,
                              const std::string module = "global") {
        lock_data_pool();
        char *ptr = shm_param_api_->GetDataPoolPtr(module, key);
        ...
        attr.SData.isVecString_size =
            value.size() < DATA_POOL_STRING_VEC_SIZE ? value.size() : DATA_POOL_STRING_VEC_SIZE;
...
private:
    bool ExeLinuxCmd(const std::string &cmd) {
        ...
        if (WIFEXITED(status)) {
            ...
            AINFO << "ExeLinuxCmdBlock " << cmd << "error returned " << WEXITSTATUS(status);
            return false;
        }
        return true;
    }
    ...
    SHMParamApi() {
        ...
        lock_data_pool();
        current_shmid = avos::common::SharedMemory::sharedMemoryCreateOrGet(
            avos::common::SharedMemory::BKDRHash("avos_shm_param_pool_112"),
            sizeof(MultiModuleShmPool) / 4096 * 4096 + 4096);
        // AINFO << "current_shmid:" << current_shmid ;
        if (current_shmid == -1) {
            AINFO << "1 error to alloc current_shmid " << current_shmid;
            exit(0);
        }
        ...
        bool bCreate = false;
        data_pool_shmid = avos::common::SharedMemory::sharedMemoryCreateOrGet(
            avos::common::SharedMemory::BKDRHash("avos_data_pool"), sizeof(CDataPoolbuf) / 4096 * 4096 + 4096, bCreate);
        // AINFO << "data_pool_shmid:" << data_pool_shmid ;
        if (data_pool_shmid == -1) {
            AINFO << "error to alloc data_pool_shmid " << data_pool_shmid;
            exit(0);
        }
        ...
        // AINFO << "data_pool_str:"<<data_pool_str<<std::endl;
        fflush(stdout);
        unlock_data_pool();
    }
