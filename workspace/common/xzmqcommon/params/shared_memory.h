...
namespace avos {
    namespace common {
        ...
        typedef struct ShmHeader {
            ...
            void PrintInfo() {
                AINFO << "shm info : " << shm_attach_times.load();
                AINFO << "lowest_loc:" << lowest_loc.array_index << "|" << lowest_loc.seq_num;
                AINFO << "highest_loc:" << highest_loc.array_index << "|" << highest_loc.seq_num;
            }
        } ShmHeader;
        ...
        public:
            ...
            static int sharedMemoryCreate(key_t key, int size) {
                ...
                if (shmid != -1) {
                    ...
                    if (shmid != -1) {
                        ...
                        if (buf.shm_segsz < size) {
                            // AINFO << "shm exist need size = " << size << ", current size = " << buf.shm_segsz ;
                ...
                if (shmid == -1) {
                    AINFO << "shmid = -1, errno = " << errno;
                } else {
                    char *to_mem = (char *)sharedMemoryAttach(shmid);
                    memset(to_mem, 0, size);
                    // AINFO << "create shm sucess key = " << key << ", size = " << size ;
                }
                return shmid;
            }
            ...
            static int sharedMemoryCreateOrGet(key_t key, int size, bool &bCreate) {
                ...
                if (shmid == -1) {
                    ...
                    // AINFO << "get or create shm sucess key = " << key << ", size = " << size ;
                }
                return shmid;
            }
            ...
            static int ResizeSharedMemory(const int &shmid, const int &key, const int &size_bak,
