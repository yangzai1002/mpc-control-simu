...
public:
    ...
    CAvosParamWebClient() : m_open(false), m_done(false) {
        ...
        m_client.set_message_handler(bind(&CAvosParamWebClient::on_message, this, websocketpp::lib::placeholders::_1,
        ...
        if (res_ver.size() >= 6) {
            ...
            if (std::string(param_type) == "-s") {
                if (CShmBaseFunc::Instance()->isContain(string(param_value), ",")) {
                    ...
                    if (shm_ptr->SetStringModuleParam(std::string(param_name), std::string(param_value),
