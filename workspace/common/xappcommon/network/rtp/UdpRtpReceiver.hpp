#ifndef NET_UDP_RTP_RECEIVER_HPP
#define NET_UDP_RTP_RECEIVER_HPP
...
public:
    UdpRtpReceiver(std::uint32_t ssrc, const std::string &peer_ip, std::uint16_t peer_rtp_port,
                   std::uint16_t peer_rtcp_port)
        : RtpReceiver(ssrc), _rtp_peer(peer_ip, peer_rtp_port), _rtcp_peer(peer_ip, peer_rtcp_port) {
    }

    ~UdpRtpReceiver() override {
    ...
    [[nodiscard]] std::string string() const override {
        return fmt::format("rtp {}, rtcp {}", _rtp_peer.string(), _rtcp_peer.string());
    }
    ...
    void sendmesg(std::int32_t fd, const sockaddr_in &peer, const std::uint8_t *p, std::size_t s) noexcept(false) {
        ...
        retry:
            ...
            if (n < 0) {
                ...
                throw std::runtime_error("sendto error-" + app_common::StringUtils::str_error(_error));
            }
    ...
    void sendmesg(std::int32_t fd, const sockaddr_in &peer, const std::uint8_t *p1, std::size_t s1,
        ...
        if (r != s1 + s2) {
            error(errno);
            throw std::runtime_error("sendmsg error with " + app_common::StringUtils::str_error(_error));
        }
    }
