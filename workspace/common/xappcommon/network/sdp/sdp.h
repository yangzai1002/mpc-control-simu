...
#define GET_BANDWIDTH_INFO(bw)                                               \
    do {                                                                     \
        ...
        while (key == 'b') {                                                 \
            ADD_ENTRY(bw);                                                   \
            n = bw##_count - 1;                                              \
            split_values(value, ':', "ss", &bw[n].bwtype, &bw[n].bandwidth); \
            p = load_next_entry(p, &key, &value);                            \
        }                                                                    \
    } while (0)
...
#define ADD_ENTRY(field)                                                 \
    do {                                                                 \
        field##_count++;                                                 \
        if (!field) {                                                    \
            field = (decltype(field))calloc(1, sizeof(*field));          \
        } else {                                                         \
            int n = field##_count;                                       \
            field = (decltype(field))realloc(field, sizeof(*field) * n); \
...
static void sdp_destroy(struct sdp_payload *sdp) {
    size_t i, j;
...
static struct sdp_payload *sdp_parse(const char *payload) {
    struct sdp_payload *sdp = (sdp_payload *)calloc(1, sizeof(*sdp));
    char *              p, key, *value;
    ...
    if (key != 'o')
        ...
        auto *o = &sdp->origin;
        split_values(value, ' ', "sllsss", &o->username, &o->sess_id, &o->sess_version, &o->nettype, &o->addrtype,
                     &o->addr);
    }
    ...
    do {
        ADD_ENTRY(sdp->times);
        auto *tf = &sdp->times[sdp->times_count - 1];
        split_values(value, ' ', "tt", &tf->start_time, &tf->stop_time);
        p = load_next_entry(p, &key, &value);

        while (key == 'r') {
            ADD_ENTRY(tf->repeat);
            auto *rf = &tf->repeat[tf->repeat_count - 1];
    ...
    if (key == 'z') {
        while (*value) {
            ...
            auto *za = &sdp->zone_adjustments[n];
    ...
    while (key == 'm') {
        ...
        auto *md = &sdp->medias[sdp->medias_count - 1];
...
static char *sdp_get_attr(char **attr, size_t nattr, char *key) {
...
static int sdp_has_flag_attr(char **attr, size_t nattr, char *flag) {
    size_t i;
...
static void sdp_dump(struct sdp_payload *sdp) {
    size_t i, j, k;
    ...
    if (sdp->zone_adjustments_count) {
        ...
        for (i = 0; i < sdp->zone_adjustments_count; i++)
            printf("%ld %ld%s", sdp->zone_adjustments[i].adjust, sdp->zone_adjustments[i].offset,
