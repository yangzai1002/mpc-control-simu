...
#include "Waypoint.h"
#include "ara/adsfi/impl_type_idpbusinesspoint.h"
...
namespace app_common {
    ...
    class ExceptionTooFarawayFromBeginToEnd : public std::exception {
    public:
        const std::string track_file;
        const double      distance;
        ExceptionTooFarawayFromBeginToEnd(const std::string& track_file_, double distance_)
            : std::exception(), track_file(track_file_), distance(distance_) {
        }
    };
    ...
    class ExceptionLoopTrackTooShort : public std::exception {
    public:
        const std::string track_file;
        const double      distance;
        ExceptionLoopTrackTooShort(const std::string& track_file_, double distance_)
            : std::exception(), track_file(track_file_), distance(distance_) {
        }
    };
        ...
        std::vector<P> smooth(const std::vector<P>& input_path) {
            ...
            for (int iter = 0; iter < max_iterations_; ++iter) {
                ...
                if (max_change < convergence_threshold_) {
                    AINFO << "迭代" << iter + 1 << "次后收敛，最大变化: " << max_change;
                    break;
                }
            }
    ...
    public:
        ...
        virtual std::vector<std::vector<ara::adsfi::IdpBusinessPoint>> paths() = 0;
        ...
        void smooth_waypoints(std::vector<Waypoint>& waypoints) {
            ...
            std::vector<ara::adsfi::IdpBusinessPoint> local_path;
            std::for_each(pgccs_l.begin(), pgccs_l.end(), [&local_path](const GeoPointGCCS& p) {
                ara::adsfi::IdpBusinessPoint bp{};
                bp.x = p.xg;
                bp.y = p.yg;
                local_path.push_back(bp);
            });

            AINFO << "smooth " << local_path.size() << " points begin ...";
            AdaptiveCurvatureSmoother<ara::adsfi::IdpBusinessPoint> smoother(0.4, // 高曲率阈值
                                                                             0.5, // 高曲率区域平滑强度
                                                                             0.6, // 低曲率区域平滑强度
                                                                             10,  // 窗口大小
                                                                             20,  // 最大迭代次数
            ...
            AINFO << "smooth " << smoothed_path.size() << " points end ...";
            ...
            AINFO << "smooth_waypoints--new waypoints count: " << new_waypoints.size();
        ...
        void smooth_waypoints(std::vector<Waypoint>& waypoints) {
            ...
            std::for_each(pgccs_l.begin(), pgccs_l.end(), [&lx, &ly](const avos::vectormap::PointGCCS& p) {
                lx.push_back(p.xg);
                ly.push_back(p.yg);
            });
            avos::planning::DiscretePointsReferenceLineSmoother smoother;
            std::vector<double>                                 sx;
            std::vector<double>                                 sy;
            smoother.Smooth(lx, ly, sx, sy);
            std::vector<ReferenceLine::path_point> smooth_path;
            AINFO << "smooth_waypoints--waypoints count: " << sx.size() << ", " << sy.size();
            for (int i = 0; i < pgccs_l.size(); ++i) {
                // pgccs_l[i].xg = sx[i];
                // pgccs_l[i].yg = sy[i];
                ReferenceLine::path_point p;
                p.x = sx[i];
                p.y = sy[i];
            }

#else
            std::vector<ReferenceLine::path_point> local_path;
            std::for_each(pgccs_l.begin(), pgccs_l.end(), [&local_path](const avos::vectormap::PointGCCS& p) {
                ReferenceLine::path_point bp{};
                bp.x = p.xg;
                bp.y = p.yg;
                local_path.push_back(bp);
            });
            ...
            param["ref_weight_smooth"] = 10000000.0;
            param["ref_weight_path_length"] = 5.0;
            param["ref_weight_ref_deviation"] = 5.5;
            param["x_lower_bound"] = -0.25;
            param["x_upper_bound"] = 0.25;
            param["y_lower_bound"] = -0.25;
            param["y_upper_bound"] = 0.25;
            ReferenceLine reference_line(0, param);
            auto          smooth_path = reference_line.discrete_smooth(local_path);
            pgccs_l.clear();
#endif
            AINFO << __FUNCTION__ << ": smooth_path size: " << smooth_path.size();
            for (std::int32_t i = 0; i < smooth_path.size(); ++i) {
                auto&                      p = smooth_path[i];
                GeoPointGCCS pgccs;
                pgccs.xg = p.x;
                pgccs.yg = p.y;
                pgccs_l.push_back(pgccs);
                if (i < 1000) {
                    // AINFO << __FUNCTION__ << ": smooth_path, xg= " << std::setprecision(20) << p.x << ", yg= " <<
                    // p.y;
                }
            }
