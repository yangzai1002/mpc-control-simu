#ifndef COMMON_UTILS_SAFEQUEUE_HPP
#define COMMON_UTILS_SAFEQUEUE_HPP
...
namespace app_common {
    ...
    template <class T, class Container = std::list<T>> class SafeQueue {
    ...
    public:
         ...
         * that type has no move assignment operator. If queue is empty, this
         * function blocks until item becomes available. \param[out] item The item.
         */
         ...
         * try_pop() method if that type has no move assignment operator.
         * \param[out] item The item. \return False is returned if no item is
         * available.
         */
        ...
        bool timeout_pop_latest(value_type& item, std::uint64_t timeout) {
            std::unique_lock<std::mutex> lock(m_mutex);
         ...
         * microseconds, or until item becomes available. This method is identical
         * to the try_pop() method if that type has no move assignment operator.
         * \param[out] t An item.
         * \param[in] timeout The number of microseconds to wait.
         * \return true if get an item from the queue, false if no item is received
         * before the timeout.
        ...
        /**
         *  insert a content at front.
         * \param[in] item The item to insert at head.
         */
        bool insert_head(value_type&& item) {
            std::lock_guard<std::mutex> lock(m_mutex);
            if (m_max_num_items > 0 && m_queue.size() > m_max_num_items)
                return false;
    ...
    template <class T, class Container> void swap(SafeQueue<T, Container>& q1, SafeQueue<T, Container>& q2) {
        q1.swap(q2);
    }
};     // namespace app_common
