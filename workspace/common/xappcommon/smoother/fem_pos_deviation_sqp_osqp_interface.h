...
namespace avos::planning {
    ...
    public:
        ...
        bool Solve() {
            ...
            CalculateAffineConstraint(ref_points_, &A_data, &A_indices, &A_indptr, &lower_bounds, &upper_bounds);
            ...
            auto* settings = reinterpret_cast<OSQPSettings*>(c_malloc(sizeof(OSQPSettings)));
            ...
            while (pen_itr < sqp_pen_max_iter_) {
                ...
                while (sub_itr < sqp_sub_max_iter_) {
                    ...
                    CalculateOffset(&q);
                    CalculateAffineConstraint(opt_xy_, &A_data, &A_indices, &A_indptr, &lower_bounds, &upper_bounds);
    ...
    private:
        void CalculateKernel(std::vector<c_float>* P_data, std::vector<c_int>* P_indices,
            ...
            for (int col = 0; col < 2; ++col) {
                columns[col].emplace_back(col, weight_fem_pos_deviation_ + weight_path_length_ + weight_ref_deviation_);
                ++col_num;
            }
            ...
            for (int col = 2; col < 4; ++col) {
                ...
                columns[col].emplace_back(
                    col, 5.0 * weight_fem_pos_deviation_ + 2.0 * weight_path_length_ + weight_ref_deviation_);
                ++col_num;
            }
            ...
            for (int point_index = 2; point_index < second_point_from_last_index; ++point_index) {
                ...
                for (int col = 0; col < 2; ++col) {
                    ...
                    columns[col_index].emplace_back(
                        col_index, 6.0 * weight_fem_pos_deviation_ + 2.0 * weight_path_length_ + weight_ref_deviation_);
                    ++col_num;
                }
            }
        ...
        static std::vector<double> CalculateLinearizedFemPosParams(const std::vector<std::pair<double, double>>& points,
                                                                   size_t index) {
            //    CHECK_GT(index, 0);
            ...
            double linear_approx = (-2.0 * x_m + x_f + x_l) * (-2.0 * x_m + x_f + x_l) +
                                   (-2.0 * y_m + y_f + y_l) * (-2.0 * y_m + y_f + y_l) + -x_f * linear_term_x_f +
                                   -x_m * linear_term_x_m + -x_l * linear_term_x_l + -y_f * linear_term_y_f +
        ...
        double CalculateConstraintViolation(const std::vector<std::pair<double, double>>& points) {
            ...
            for (size_t i = 1; i < points.size() - 1; ++i) {
                ...
                double cviolation = curvature_constraint_sqr - (-2.0 * x_m + x_f + x_l) * (-2.0 * x_m + x_f + x_l) +
