...
namespace Kalman {
  ...
  void EKFilter<T, BEG, OQ, OVR, DBG>::setSizeZ(K_UINT_32 m_) {
    flags |= KALMAN_M_MODIFIED; // lxf - 20170428
    if (m_ != m) {
  ...
  void EKFilter<T, BEG, OQ, OVR, DBG>::factor(Matrix& P_) {
    ...
    // ne pas v�rifier que P_.ncol() == P_.nrow(), comme �a, m�me si
    // nrow() < ncol(), on peut factoriser la sous-matrice carr�e de P
  ...
  void EKFilter<T, BEG, OQ, OVR, DBG>::timeUpdate() {
   ...
   // cout << "in timeUpdate " << U.nrow()<< "|" <<  U.ncol() << "|"<< A.nrow()<< "|" <<  A.ncol()<<endl;
    // U = phi * U
    // d = diag(U)
    // 
