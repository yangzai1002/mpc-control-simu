...
public:
     ...
     *
     */
    void solve(avos::perception::xfusion_new::Matrix<Data> &m) {
        const size_t rows = m.rows(),
                columns = m.columns(),
                size = std::max(rows, columns);

#ifdef DEBUG
        AINFO  << "Munkres input: " << m;
#endif
    ...
    static void replace_infinites(avos::perception::xfusion_new::Matrix<Data> &matrix) {
    ...
    static void minimize_along_direction(avos::perception::xfusion_new::Matrix<Data> &matrix, const bool over_columns) {
...
private:
  ...
  #ifdef DEBUG
      AINFO  << "Final cover count: " << covercount;
  #endif
      return 0;
    }

  #ifdef DEBUG
    AINFO  << "Munkres matrix has " << covercount << " of " << matrix.minsize() << " Columns covered:";
    AINFO  << matrix;
  #endif
  ...
  avos::perception::xfusion_new::Matrix<int> mask_matrix;
  avos::perception::xfusion_new::Matrix<Data> matrix;
  bool *row_mask;
  bool *col_mask;
