...
#include "common_math/line_segment2d.h"
#include "common_math/vec2d.h"

/**
 * @namespace avos::common::math
 * @brief avos::common::math
 */
namespace avos {
namespace common {
namespace math {
...
public:
  ...
  static bool IsValidInput(const Box2d &box);

  static bool IsValidInput(const std::vector<Vec2d>& input);
  ...
  /**
   * @brief Get the probability of the polygon.
   * @return The probability of the polygon.
   */
  double probability() const { return probability_; }
  void set_p(double &p) {probability_ = p; }
  ...
  double DistanceTo(const Vec2d &point) const;
    ...
    /** 
   * @brief Compute range of l when x = s.
   * @param point The s to compute whose l range to the polygon.
   * @return whether has l range.
   */
  bool VerticalDistanceTo(const double &s, std::pair<double, double>& l_range) const;
