...
//
#ifndef MSGHANDLE_V2_PARSE_HPP
#define MSGHANDLE_V2_PARSE_HPP
...
public:
    context()
        :m_trail(0), m_cs(MSGHANDLE_CS_HEADER)
    {
    }
...
private:
    ...
    struct array_sv {
        ...
        msghandle_container_type type() const { return MSGHANDLE_CT_ARRAY_ITEM; }
    private:
    ...
    struct map_sv {
        ...
        msghandle_container_type type() const { return MSGHANDLE_CT_MAP_KEY; }
    private:
        VisitorHolder& m_visitor_holder;
    };
    struct map_ev {
    ...
    struct unpack_stack {
        struct stack_elem {
            stack_elem(msghandle_container_type type, uint32_t rest):m_type(type), m_rest(rest) {}
            msghandle_container_type m_type;
            uint32_t m_rest;
        };
        unpack_stack() {
            m_stack.reserve(MSGHANDLE_EMBED_STACK_SIZE);
        }
        parse_return push(VisitorHolder& visitor_holder, msghandle_container_type type, uint32_t rest) {
        ...
        parse_return consume(VisitorHolder& visitor_holder) {
            while (!m_stack.empty()) {
                ...
                case MSGHANDLE_CT_MAP_KEY:
                    ...
                    e.m_type = MSGHANDLE_CT_MAP_VALUE;
                    return PARSE_CONTINUE;
                case MSGHANDLE_CT_MAP_VALUE:
...
public:
     ...
     *              See https://github.com/msghandle/msghandle-c/wiki/v2_0_cpp_visitor#parse-api
...
inline bool parse(const char* data, size_t len, size_t& off, Visitor& v) {
    parse_return ret = msghandle::detail::parse_imp(data, len, off, v);
...
inline bool parse(const char* data, size_t len, Visitor& v) {
    std::size_t off = 0;
    return msghandle::parse(data, len, off, v);
}
