...
//
#ifndef MSGHANDLE_V1_ZBUFFER_HPP
#define MSGHANDLE_V1_ZBUFFER_HPP
...
public:
    zbuffer(int level = Z_DEFAULT_COMPRESSION,
            size_t init_size = MSGHANDLE_ZBUFFER_INIT_SIZE)
        : m_data(MSGHANDLE_NULLPTR), m_init_size(init_size)
    {
...
public:
    void write(const char* buf, size_t len)
    {
        ...
        while(m_stream.avail_in > 0) {
            if(m_stream.avail_out < MSGHANDLE_ZBUFFER_RESERVE_SIZE) {
                if(!expand()) {
                    throw std::bad_alloc();
                }
            }
    ...
    char* release_buffer()
    {
        ...
        m_stream.next_out = MSGHANDLE_NULLPTR;
        m_stream.avail_out = 0;
        return tmp;
    }
...
#endif // MSGHANDLE_V1_ZBUFFER_HPP
