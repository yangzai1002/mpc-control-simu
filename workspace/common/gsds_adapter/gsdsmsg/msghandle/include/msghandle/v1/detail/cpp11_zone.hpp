...
//
#ifndef MSGHANDLE_CPP11_ZONE_HPP
#define MSGHANDLE_CPP11_ZONE_HPP

#include "msghandle/versioning.hpp"
#include "msghandle/cpp_config.hpp"
#include "msghandle/zone_decl.hpp"
...
private:
    ...
    struct finalizer_array {
        finalizer_array():m_tail(MSGHANDLE_NULLPTR), m_end(MSGHANDLE_NULLPTR), m_array(MSGHANDLE_NULLPTR) {}
        void call() {
        ...
        {
            other.m_tail = MSGHANDLE_NULLPTR;
            other.m_end = MSGHANDLE_NULLPTR;
            other.m_array = MSGHANDLE_NULLPTR;
        }
    ...
    struct chunk_list {
        ...
        void clear(size_t chunk_size)
        {
            ...
            m_head->m_next = MSGHANDLE_NULLPTR;
            m_free = chunk_size;
        ...
        {
            other.m_head = MSGHANDLE_NULLPTR;
        }
...
public:
    zone(size_t chunk_size = MSGHANDLE_ZONE_CHUNK_SIZE);

public:
    void* allocate_align(size_t size, size_t align = MSGHANDLE_ZONE_ALIGN);
    void* allocate_no_align(size_t size);

    void push_finalizer(void (*func)(void*), void* data);

    template <typename T>
    void push_finalizer(msghandle::unique_ptr<T> obj);
...
{
    MSGHANDLE_ASSERT(align != 0 && (align & (align - 1)) == 0); // align must be 2^n (n >= 0)
    return
...
template <typename T>
inline void zone::push_finalizer(msghandle::unique_ptr<T> obj)
{
...
{
    void* x = allocate_align(sizeof(T), MSGHANDLE_ZONE_ALIGNOF(T));
    try {
...
#endif // MSGHANDLE_CPP11_ZONE_HPP
