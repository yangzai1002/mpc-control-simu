...
//
#ifndef MSGHANDLE_V1_PACK_HPP
#define MSGHANDLE_V1_PACK_HPP
...
public:
     ...
     */
    template <typename T>
    packer<Stream>& pack(const T& v,SMsgHandleInfo& msghandle_info);

    template <typename T>
    packer<Stream>& print(const T& v,const std::string& k);

    template <typename T>
    packer<Stream>& checkcode(const T& v,const std::string& k);

    template <typename T>
    packer<Stream>& reflectget(const T& v,const std::string& k, const std::string filter);
     ...
     * See https://github.com/msghandle/msghandle/blob/master/spec.md#formats-int
     *
     ...
     * See https://github.com/msghandle/msghandle/blob/master/spec.md#formats-float
     *
     * @param d a packing object.
     *
     * @return The reference of `*this`.
     ...
     * See https://github.com/msghandle/msghandle/blob/master/spec.md#formats-nil
     *
     * @return The reference of `*this`.
     */
     ...
     * See https://github.com/msghandle/msghandle/blob/master/spec.md#bool-format-family
     *
     * @return The reference of `*this`.
     */
     ...
     * You need to pack `n` msghandle objects following this header and size.
     * See https://github.com/msghandle/msghandle/blob/master/spec.md#array-format-family
     *
     ...
     * You need to pack `n` pairs of msghandle objects following this header and size.
     * See https://github.com/msghandle/msghandle/blob/master/spec.md#map-format-family
     *
     * @param n The number of array elements (array size).
     *
     * @return The reference of `*this`.
     ...
     * See https://github.com/msghandle/msghandle/blob/master/spec.md#formats-str
     *
     ...
     * See https://github.com/msghandle/msghandle/blob/master/spec.md#bin-format-family
     *
     * @param l The length of string.
     *
     * @return The reference of `*this`.
     ...
     * See https://github.com/msghandle/msghandle/blob/master/spec.md#formats-ext
     *
     * @param l The length of string.
     *
     * @return The reference of `*this`.
 ...
 */
template <typename Stream, typename T>
inline void pack(Stream& s, const T& v,SMsgHandleInfo& msghandle_info)
{
    packer<Stream>(s).pack(v,msghandle_info);
}


template <typename Stream, typename T>
inline void print(Stream* s, const T& v)
{
    packer<Stream>(*s).print(v);
}
template <typename Stream, typename T>
inline void print(Stream& s, const T& v)
{
    packer<Stream>(s).print(v,"msg");
}

template <typename Stream, typename T>
inline void reflectget(Stream& s, const T& v, const std::string filter)
{
    packer<Stream>(s).reflectget(v,"msg",filter);
}

template <typename output,typename T>
inline bool reflectget(const std::string filter,output& o, const T& v)
{
    bool breflectsucess = true;
    std::stringstream buffer;
    std::string filterall = "msg."+filter;
    msghandle::v1::reflectget(buffer,v,filterall);
    std::string str = std::move(buffer.str());
    if(str.size() == 0)
        breflectsucess = false;
    else
        buffer>>o;
    return breflectsucess;
}



template <typename T>
inline void checkcode(std::string& s, const T& v)
{
//    packer<std::stringstream>(s).checkcode(v," ");
    std::stringstream buffer;

    msghandle::v1::checkcode(buffer, v);
//    std::cout<< buffer.str() <<std::endl;
    unsigned char result[SHA256_DIGEST_LENGTH];
    memset(result,'\0',SHA256_DIGEST_LENGTH);
    SHA256((unsigned char*)buffer.str().c_str(), buffer.str().length(), result);
...
template <typename Stream, typename T>
inline void checkcode(Stream& s, const T& v)
{
    packer<Stream>(s).checkcode(v," ");
}
...
{
    char buf[3];
    buf[0] = static_cast<char>(0xcdu); _msghandle_store16(&buf[1], d);
    append_buffer(buf, 3);
    return *this;
}
...
{
    char buf[5];
    buf[0] = static_cast<char>(0xceu); _msghandle_store32(&buf[1], d);
    append_buffer(buf, 5);
    return *this;
}
...
{
    char buf[9];
    buf[0] = static_cast<char>(0xcfu); _msghandle_store64(&buf[1], d);
    append_buffer(buf, 9);
    return *this;
}
...
{
    char buf[3];
    buf[0] = static_cast<char>(0xd1u); _msghandle_store16(&buf[1], (uint16_t)d);
    append_buffer(buf, 3);
    return *this;
}
...
{
    char buf[5];
    buf[0] = static_cast<char>(0xd2u); _msghandle_store32(&buf[1], (uint32_t)d);
    append_buffer(buf, 5);
    return *this;
}
...
{
    char buf[9];
    buf[0] = static_cast<char>(0xd3u); _msghandle_store64(&buf[1], d);
    append_buffer(buf, 9);
    return *this;
}
...
inline packer<Stream>& packer<Stream>::pack_float(float d)
{
    ...
    mem.f = d;
    char buf[5];
    buf[0] = static_cast<char>(0xcau); _msghandle_store32(&buf[1], mem.i);
    append_buffer(buf, 5);
    return *this;
}
...
#endif
    _msghandle_store64(&buf[1], mem.i);
    append_buffer(buf, 9);
    return *this;
}
...
inline packer<Stream>& packer<Stream>::pack_array(uint32_t n)
{
    if(n < 16) {
        ...
        char buf[3];
        buf[0] = static_cast<char>(0xdcu); _msghandle_store16(&buf[1], static_cast<uint16_t>(n));
        append_buffer(buf, 3);
    } else {
        char buf[5];
        buf[0] = static_cast<char>(0xddu); _msghandle_store32(&buf[1], static_cast<uint32_t>(n));
        append_buffer(buf, 5);
    }
    return *this;
}
...
inline packer<Stream>& packer<Stream>::pack_map(uint32_t n)
{
    if(n < 16) {
        ...
        char buf[3];
        buf[0] = static_cast<char>(0xdeu); _msghandle_store16(&buf[1], static_cast<uint16_t>(n));
        append_buffer(buf, 3);
    } else {
        char buf[5];
        buf[0] = static_cast<char>(0xdfu); _msghandle_store32(&buf[1], static_cast<uint32_t>(n));
        append_buffer(buf, 5);
    }
    return *this;
}
...
inline packer<Stream>& packer<Stream>::pack_str(uint32_t l)
{
    if(l < 32) {
        ...
        char buf[3];
        buf[0] = static_cast<char>(0xdau); _msghandle_store16(&buf[1], static_cast<uint16_t>(l));
        append_buffer(buf, 3);
    } else {
        char buf[5];
        buf[0] = static_cast<char>(0xdbu); _msghandle_store32(&buf[1], static_cast<uint32_t>(l));
        append_buffer(buf, 5);
    }
    return *this;
}
...
inline packer<Stream>& packer<Stream>::pack_bin(uint32_t l)
{
    if(l < 256) {
        ...
        char buf[3];
        buf[0] = static_cast<char>(0xc5u); _msghandle_store16(&buf[1], static_cast<uint16_t>(l));
        append_buffer(buf, 3);
    } else {
        char buf[5];
        buf[0] = static_cast<char>(0xc6u); _msghandle_store32(&buf[1], static_cast<uint32_t>(l));
        append_buffer(buf, 5);
    }
    return *this;
}
...
inline packer<Stream>& packer<Stream>::pack_ext(size_t l, int8_t type)
{
    ...
    default:
        if(l < 256) {
            ...
            _msghandle_store16(&buf[1], static_cast<uint16_t>(l));
            buf[3] = static_cast<char>(type);
            append_buffer(buf, 4);
        } else {
            char buf[6];
            buf[0] = static_cast<char>(0xc9u);
            _msghandle_store32(&buf[1], static_cast<uint32_t>(l));
...
inline void packer<Stream>::pack_imp_uint16(T d)
{
    if(d < (1<<7)) {
        ...
        /* unsigned 16 */
        char buf[3];
        buf[0] = static_cast<char>(0xcdu); _msghandle_store16(&buf[1], static_cast<uint16_t>(d));
        append_buffer(buf, 3);
    }
}
...
inline void packer<Stream>::pack_imp_uint32(T d)
{
    if(d < (1<<8)) {
        ...
        if(d < (1<<16)) {
            ...
            /* unsigned 32 */
            char buf[5];
            buf[0] = static_cast<char>(0xceu); _msghandle_store32(&buf[1], static_cast<uint32_t>(d));
            append_buffer(buf, 5);
        }
    }
}
...
inline void packer<Stream>::pack_imp_int16(T d)
{
    if(d < -(1<<5)) {
        if(d < -(1<<7)) {
            /* signed 16 */
            char buf[3];
            buf[0] = static_cast<char>(0xd1u); _msghandle_store16(&buf[1], static_cast<int16_t>(d));
            append_buffer(buf, 3);
        } else {
            /* signed 8 */
...
inline void packer<Stream>::pack_imp_int32(T d)
{
    if(d < -(1<<5)) {
        if(d < -(1<<15)) {
            /* signed 32 */
            char buf[5];
            buf[0] = static_cast<char>(0xd2u); _msghandle_store32(&buf[1], static_cast<int32_t>(d));
            append_buffer(buf, 5);
        } else if(d < -(1<<7)) {
            /* signed 16 */
            char buf[3];
