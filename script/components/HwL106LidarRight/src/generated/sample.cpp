...
bool Sample::Init()
{
    	ApLogInterface::Instance()->Init("HwL106LidarRight");
...
void Sample::SensorLidarPointCloudServiceInterfaceServerRun()
{
    ...
    while (workFlag_) {
		...
		AVOS_MSGPACK_PUBLISH("HwL106LidarRight",portName, "MsgLidarFrame" , static_cast<void *>(data.get()));
...
}void Sample::SensorLidarPacketServiceInterfaceServerRun()
{
    ...
    while (workFlag_) {
		...
		AVOS_MSGPACK_PUBLISH("HwL106LidarRight",portName, "SensorLidarPacket" , static_cast<void *>(data.get()));
