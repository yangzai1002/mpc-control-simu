...
bool Sample::Init()
{
    	ApLogInterface::Instance()->Init("HwL106LidarLeft");
...
void Sample::SensorLidarPointCloudServiceInterfaceServerRun()
{
    ...
    while (workFlag_) {
		...
		AVOS_MSGPACK_PUBLISH("HwL106LidarLeft",portName, "MsgLidarFrame" , static_cast<void *>(data.get()));
...
}void Sample::SensorLidarPacketServiceInterfaceServerRun()
{
    ...
    while (workFlag_) {
		...
		AVOS_MSGPACK_PUBLISH("HwL106LidarLeft",portName, "SensorLidarPacket" , static_cast<void *>(data.get()));
