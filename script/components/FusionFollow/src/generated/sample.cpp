...
bool Sample::Init()
{
    	ApLogInterface::Instance()->Init("FusionFollow");
...
void Sample::FusionObjectArrayIntfServerRun()
{
    ...
    while (workFlag_) {
        ...
        for (const auto& portName : fusionObjectArrayIntfServerPortVec) {
			...
			PRE_TIMMER_SENTENCE
		auto code_tag_key = portName +"_FusionObjectArrayData";
		...
		AVOS_MSGPACK_PUBLISH("FusionFollow",portName, "MsgHafFusionOutArray" , static_cast<void *>(data.get()));
