...
bool Sample::Init()
{
    	ApLogInterface::Instance()->Init("LidarSlam");
...
void Sample::LocationInfoIntfServerRun()
{
    ...
    while (workFlag_) {
        ...
        for (const auto& portName : locationInfoIntfServerPortVec) {
			...
			PRE_TIMMER_SENTENCE
		auto code_tag_key = portName +"_LocationInfoData";
		...
		AVOS_MSGPACK_PUBLISH("LidarSlam",portName, "MsgHafLocation" , static_cast<void *>(data.get()));
