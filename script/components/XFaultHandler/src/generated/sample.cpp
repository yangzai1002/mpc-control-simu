...
namespace mdc {
Sample::Sample(): swcPtr_(std::make_unique<mdc::xfault_handler_base::XfaultHandlerBaseSwC>()), workFlag_(true)
{}
...
void Sample::Run()
{
    FaultDataServiceInterfaceServerThreadPtr_ =
        std::make_unique<std::thread>(std::bind(&Sample::FaultDataServiceInterfaceServerRun, this));
...
void Sample::FaultDataServiceInterfaceServerRun()
{
    if (!swcPtr_) {
    ...
    auto faultDataServiceInterfaceServerPortVec = swcPtr_->GetFaultDataServiceInterfaceServerVec();
		pthread_setname_np(pthread_self(),"Process:FaultData");

    while (workFlag_) {
        /* 遍历port name，获取服务端对象并发送数据 */
        for (const auto& portName : faultDataServiceInterfaceServerPortVec) {
            if (!workFlag_) {
                return;
            }
            /* 获取发送端服务，其中portName对应MMC上配置的此应用对应的发送端portName <关键接口> */
            auto serverPtr = swcPtr_->GetFaultDataServiceInterfaceServer(portName);
            if (!serverPtr) {
                SPL_LOG_SPACE::GetLoggerIns("SPL")->LogError()<< "Failed to initialize the instance: " << portName;
                continue;
            }
			PRE_TIMMER_SENTENCE
		auto code_tag_key = portName +"_EventFaultDataData";

            auto data = std::make_shared<ara::adsfi::EventFaultDataDataType>();
		...
		AVOS_CODETAG_ADV1(code_tag_key.c_str(), 0, eAvosCodetagPubtagId);
            serverPtr->SendEventFaultDataData(data);
		AVOS_MSGPACK_PUBLISH("XFaultHandler",portName, "FaultData" , static_cast<void *>(data.get()));
...
{
    workFlag_ = false;
    if ((FaultDataServiceInterfaceServerThreadPtr_ != nullptr) &&
        (FaultDataServiceInterfaceServerThreadPtr_->joinable())) {
        FaultDataServiceInterfaceServerThreadPtr_->join();
    }
    FaultDataServiceInterfaceServerThreadPtr_ = nullptr;
