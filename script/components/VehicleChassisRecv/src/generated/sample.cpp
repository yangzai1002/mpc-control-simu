...
bool Sample::Init()
{
    	ApLogInterface::Instance()->Init("VehicleChassisRecv");
...
void Sample::VehicleInfomationServiceInterfaceServerRun()
{
    ...
    auto vehicleInfomationServiceInterfaceServerPortVec = swcPtr_->GetVehicleInfomationServiceInterfaceServerVec();
		pthread_setname_np(pthread_self(),"Process:VehicleInfomation");
    ...
    while (workFlag_) {
        ...
        for (const auto& portName : vehicleInfomationServiceInterfaceServerPortVec) {
			...
			PRE_TIMMER_SENTENCE
		auto code_tag_key = portName +"_EventVehicleInfoData";
		...
		AVOS_MSGPACK_PUBLISH("VehicleChassisRecv",portName, "VehicleInformation" , static_cast<void *>(data.get()));
