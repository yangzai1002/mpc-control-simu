...
bool Sample::Init()
{
    	ApLogInterface::Instance()->Init("HwFr671RadarRight");
...
void Sample::RadarPointCloudServiceInterfaceServerRun()
{
    ...
    while (workFlag_) {
		...
		AVOS_MSGPACK_PUBLISH("HwFr671RadarRight",portName, "SensorRadarPointCloud" , static_cast<void *>(data.get()));
...
}void Sample::SensorRadarPacketServiceInterfaceServerRun()
{
    ...
    while (workFlag_) {
		...
		AVOS_MSGPACK_PUBLISH("HwFr671RadarRight",portName, "SensorRadarPacket" , static_cast<void *>(data.get()));
