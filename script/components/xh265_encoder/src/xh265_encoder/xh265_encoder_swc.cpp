...
Xh265EncoderSwC::Xh265EncoderSwC()
    : initFlag_(false),
      workFlag_(true),
...
bool Xh265EncoderSwC::Init()
{
    ...
    #endif
    /* server agent */
    for (auto id : sensorImageServiceInterfaceServerIdMap_) {
        auto resultToken = ara::adsfi::skeleton::SensorImageServiceInterfaceSkeleton::Preconstruct(
        ...
        if (resultToken.HasValue()) {
            sensorImageServiceInterfaceServerInsMap_[id.first] = std::make_shared<SensorImageServiceInterfaceServer>(id.second);
        } else {
            LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Skeleton preconstruct Failed!";
            return false;
        }
        RETURN_IF_INIT_SERVICE_FAILED(
            (sensorImageServiceInterfaceServerInsMap_[id.first]!= nullptr),
            "SensorImageServiceInterfaceSkeleton",
            id.second);
    }
...
void Xh265EncoderSwC::Stop()
{
    workFlag_ = false;
    /* server agent */
    for (auto id : sensorImageServiceInterfaceServerIdMap_) {
        if (sensorImageServiceInterfaceServerInsMap_[id.first]) {
            sensorImageServiceInterfaceServerInsMap_[id.first]->Stop();
        } else {
...
std::shared_ptr<SensorImageServiceInterfaceServer> Xh265EncoderSwC::GetSensorImageServiceInterfaceServer(const ara::core::String& portName)
{
    std::lock_guard<std::mutex> lck(sensorImageServiceInterfaceServerInsMapMtx_);
    if (sensorImageServiceInterfaceServerInsMap_.find(portName) == sensorImageServiceInterfaceServerInsMap_.end()) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << portName << "does not exist.";
        return nullptr;
    }
    if (!(sensorImageServiceInterfaceServerInsMap_[portName]->Init())) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << portName << " init failed.";
        return nullptr;
    }
    return sensorImageServiceInterfaceServerInsMap_[portName];
}

bool Xh265EncoderSwC::LoadConfig()
{
    /* 通过环境变量获取配置文件路径 */
...
bool Xh265EncoderSwC::DoesFileExist(const ara::core::String& filePath)
{
...
bool Xh265EncoderSwC::IsDirType(const mode_t& fileMode)
{
    if (S_ISDIR(fileMode)) {
        return true;
    }
...
bool Xh265EncoderSwC::ParsingInstanceId(const std::unique_ptr<MdcYamlNode>& config)
{
    if (!config->HasKeyValue("xh265_encoder")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find xh265_encoder in config file!";
        return false;
    }
    auto swcConfig = (*(config.get()))["xh265_encoder"];
    /* server agent */
    if (!swcConfig.HasKeyValue("sendInstanceID")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find sendInstanceID in config file!";
        return false;
    }
    swcConfig["sendInstanceID"]["SensorImageServiceInterface"].GetValue<std::vector<uint32_t>>(
        "instanceId", sensorImageServiceInterfaceServerIdVec_);
    StableUnique(sensorImageServiceInterfaceServerIdVec_);
    swcConfig["sendInstanceID"]["SensorImageServiceInterface"].GetValue<std::vector<std::string>>(
        "portName", sensorImageServiceInterfaceServerPortVec_);
    StableUnique(sensorImageServiceInterfaceServerPortVec_);
    if (sensorImageServiceInterfaceServerIdVec_.size() != sensorImageServiceInterfaceServerPortVec_.size()) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "The port and instance mapping information is incorrect.";
        return false;
    }
    for (size_t i = 0; i < sensorImageServiceInterfaceServerIdVec_.size(); ++i) {
        sensorImageServiceInterfaceServerIdMap_[sensorImageServiceInterfaceServerPortVec_[i]] = sensorImageServiceInterfaceServerIdVec_[i];
    }
