...
#include "mdc/xh265_recorder/xh265_recorder_swc.h"
...
#include "mdc/xh265_recorder/sensorimageserviceinterface_client.h"
...
using LOG_SPACE = mdc::xh265_recorder::Xh265RecorderLogger;

Xh265RecorderSwC::Xh265RecorderSwC()
    : initFlag_(false),
      workFlag_(true),
...
std::shared_ptr<SensorImageServiceInterfaceClient> Xh265RecorderSwC::GetSensorImageServiceInterfaceClient(const ara::core::String& portName)
{
...
bool Xh265RecorderSwC::LoadConfig()
{
    /* 通过环境变量获取配置文件路径 */
...
bool Xh265RecorderSwC::DoesFileExist(const ara::core::String& filePath)
{
...
bool Xh265RecorderSwC::IsDirType(const mode_t& fileMode)
{
    if (S_ISDIR(fileMode)) {
        return true;
    }
...
bool Xh265RecorderSwC::ParsingInstanceId(const std::unique_ptr<MdcYamlNode>& config)
{
    if (!config->HasKeyValue("xh265_recorder")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find xh265_recorder in config file!";
        return false;
    }
    auto swcConfig = (*(config.get()))["xh265_recorder"];
    /* server agent */
