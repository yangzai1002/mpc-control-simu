...
MidTrackChassisRecvSwC::MidTrackChassisRecvSwC()
    : initFlag_(false),
      workFlag_(true),
...
bool MidTrackChassisRecvSwC::Init()
{
    ...
    #endif
    /* server agent */
    for (auto id : vehicleInfomationServiceInterfaceServerIdMap_) {
        auto resultToken = ara::adsfi::skeleton::VehicleInfomationServiceInterfaceSkeleton::Preconstruct(
        ...
        if (resultToken.HasValue()) {
            vehicleInfomationServiceInterfaceServerInsMap_[id.first] = std::make_shared<VehicleInfomationServiceInterfaceServer>(id.second);
        } else {
            LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Skeleton preconstruct Failed!";
            return false;
        }
        RETURN_IF_INIT_SERVICE_FAILED(
            (vehicleInfomationServiceInterfaceServerInsMap_[id.first]!= nullptr),
            "VehicleInfomationServiceInterfaceSkeleton",
            id.second);
    }
...
void MidTrackChassisRecvSwC::Stop()
{
    workFlag_ = false;
    /* server agent */
    for (auto id : vehicleInfomationServiceInterfaceServerIdMap_) {
        if (vehicleInfomationServiceInterfaceServerInsMap_[id.first]) {
            vehicleInfomationServiceInterfaceServerInsMap_[id.first]->Stop();
        } else {
...
std::shared_ptr<VehicleInfomationServiceInterfaceServer> MidTrackChassisRecvSwC::GetVehicleInfomationServiceInterfaceServer(const ara::core::String& portName)
{
    std::lock_guard<std::mutex> lck(vehicleInfomationServiceInterfaceServerInsMapMtx_);
    if (vehicleInfomationServiceInterfaceServerInsMap_.find(portName) == vehicleInfomationServiceInterfaceServerInsMap_.end()) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << portName << "does not exist.";
        return nullptr;
    }
    if (!(vehicleInfomationServiceInterfaceServerInsMap_[portName]->Init())) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << portName << " init failed.";
        return nullptr;
    }
    return vehicleInfomationServiceInterfaceServerInsMap_[portName];
}

bool MidTrackChassisRecvSwC::LoadConfig()
{
    /* 通过环境变量获取配置文件路径 */
...
bool MidTrackChassisRecvSwC::DoesFileExist(const ara::core::String& filePath)
{
...
bool MidTrackChassisRecvSwC::IsDirType(const mode_t& fileMode)
{
    if (S_ISDIR(fileMode)) {
        return true;
    }
...
bool MidTrackChassisRecvSwC::ParsingInstanceId(const std::unique_ptr<MdcYamlNode>& config)
{
    if (!config->HasKeyValue("mid_track_chassis_recv")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find mid_track_chassis_recv in config file!";
        return false;
    }
    auto swcConfig = (*(config.get()))["mid_track_chassis_recv"];
    /* server agent */
    if (!swcConfig.HasKeyValue("sendInstanceID")) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "Cannot find sendInstanceID in config file!";
        return false;
    }
    swcConfig["sendInstanceID"]["VehicleInfomationServiceInterface"].GetValue<std::vector<uint32_t>>(
        "instanceId", vehicleInfomationServiceInterfaceServerIdVec_);
    StableUnique(vehicleInfomationServiceInterfaceServerIdVec_);
    swcConfig["sendInstanceID"]["VehicleInfomationServiceInterface"].GetValue<std::vector<std::string>>(
        "portName", vehicleInfomationServiceInterfaceServerPortVec_);
    StableUnique(vehicleInfomationServiceInterfaceServerPortVec_);
    if (vehicleInfomationServiceInterfaceServerIdVec_.size() != vehicleInfomationServiceInterfaceServerPortVec_.size()) {
        LOG_SPACE::GetLoggerIns("SWC")->LogError() << "The port and instance mapping information is incorrect.";
        return false;
    }
    for (size_t i = 0; i < vehicleInfomationServiceInterfaceServerIdVec_.size(); ++i) {
        vehicleInfomationServiceInterfaceServerIdMap_[vehicleInfomationServiceInterfaceServerPortVec_[i]] = vehicleInfomationServiceInterfaceServerIdVec_[i];
    }
