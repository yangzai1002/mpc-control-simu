...
namespace mdc {
Sample::Sample(): swcPtr_(std::make_unique<mdc::lidar_terrain_map_detection::LidarTerrainMapDetectionSwC>()), workFlag_(true)
{}
...
bool Sample::Init()
{
    	ApLogInterface::Instance()->Init("LidarTerrainMapDetection");
...
void Sample::Run()
{
    PerceptionStaticEnvServiceInterfaceServerThreadPtr_ =
        std::make_unique<std::thread>(std::bind(&Sample::PerceptionStaticEnvServiceInterfaceServerRun, this));
...
void Sample::PerceptionStaticEnvServiceInterfaceServerRun()
{
    if (!swcPtr_) {
    ...
    auto perceptionStaticEnvServiceInterfaceServerPortVec = swcPtr_->GetPerceptionStaticEnvServiceInterfaceServerVec();
		pthread_setname_np(pthread_self(),"Process:PerceptionStaticEnv");

    while (workFlag_) {
        /* 遍历port name，获取服务端对象并发送数据 */
        for (const auto& portName : perceptionStaticEnvServiceInterfaceServerPortVec) {
            if (!workFlag_) {
                return;
            }
            /* 获取发送端服务，其中portName对应MMC上配置的此应用对应的发送端portName <关键接口> */
            auto serverPtr = swcPtr_->GetPerceptionStaticEnvServiceInterfaceServer(portName);
            if (!serverPtr) {
                SPL_LOG_SPACE::GetLoggerIns("SPL")->LogError()<< "Failed to initialize the instance: " << portName;
                continue;
            }
			PRE_TIMMER_SENTENCE
		auto code_tag_key = portName +"_EventPerceptionStaticEnvData";

            auto data = std::make_shared<ara::adsfi::EventPerceptionStaticEnvDataType>();
		...
		AVOS_CODETAG_ADV1(code_tag_key.c_str(), 0, eAvosCodetagPubtagId);
            serverPtr->SendEventPerceptionStaticEnvData(data);
		AVOS_MSGPACK_PUBLISH("LidarTerrainMapDetection",portName, "PerceptionStaticEnv" , static_cast<void *>(data.get()));
...
{
    workFlag_ = false;
    if ((PerceptionStaticEnvServiceInterfaceServerThreadPtr_ != nullptr) &&
        (PerceptionStaticEnvServiceInterfaceServerThreadPtr_->joinable())) {
        PerceptionStaticEnvServiceInterfaceServerThreadPtr_->join();
    }
    PerceptionStaticEnvServiceInterfaceServerThreadPtr_ = nullptr;
