...
void Sample::BagStatusServiceInterfaceServerRun()
{
    ...
    auto bagStatusServiceInterfaceServerPortVec = swcPtr_->GetBagStatusServiceInterfaceServerVec();
		pthread_setname_np(pthread_self(),"Process:BagStatus");
    ...
    while (workFlag_) {
        ...
        for (const auto& portName : bagStatusServiceInterfaceServerPortVec) {
			...
			PRE_TIMMER_SENTENCE
		auto code_tag_key = portName +"_EventBagStatusData";
		...
		AVOS_MSGPACK_PUBLISH("XDataTriggerRecorder",portName, "FaultData" , static_cast<void *>(data.get()));
