...
namespace mdc {
Sample::Sample(): swcPtr_(std::make_unique<mdc::multy_camera_det_base::MultyCameraDetBaseSwC>()), workFlag_(true)
{}
...
bool Sample::Init()
{
    	ApLogInterface::Instance()->Init("MultiCameraDetection");
...
void Sample::Run()
{
    VisionSegmentServiceInterfaceServerThreadPtr_ =
        std::make_unique<std::thread>(std::bind(&Sample::VisionSegmentServiceInterfaceServerRun, this));
...
void Sample::VisionSegmentServiceInterfaceServerRun()
{
    if (!swcPtr_) {
    ...
    auto visionSegmentServiceInterfaceServerPortVec = swcPtr_->GetVisionSegmentServiceInterfaceServerVec();
		pthread_setname_np(pthread_self(),"Process:VisionSegment");

    while (workFlag_) {
        /* 遍历port name，获取服务端对象并发送数据 */
        for (const auto& portName : visionSegmentServiceInterfaceServerPortVec) {
            if (!workFlag_) {
                return;
            }
            /* 获取发送端服务，其中portName对应MMC上配置的此应用对应的发送端portName <关键接口> */
            auto serverPtr = swcPtr_->GetVisionSegmentServiceInterfaceServer(portName);
            if (!serverPtr) {
                SPL_LOG_SPACE::GetLoggerIns("SPL")->LogError()<< "Failed to initialize the instance: " << portName;
                continue;
            }
			PRE_TIMMER_SENTENCE
		auto code_tag_key = portName +"_EventVisionSegmentData";

            auto data = std::make_shared<ara::adsfi::EventVisionSegmentDataType>();
		...
		AVOS_CODETAG_ADV1(code_tag_key.c_str(), 0, eAvosCodetagPubtagId);
            serverPtr->SendEventVisionSegmentData(data);
		AVOS_MSGPACK_PUBLISH("MultiCameraDetection",portName, "MsgCameraVPResult" , static_cast<void *>(data.get()));
...
{
    workFlag_ = false;
    if ((VisionSegmentServiceInterfaceServerThreadPtr_ != nullptr) &&
        (VisionSegmentServiceInterfaceServerThreadPtr_->joinable())) {
        VisionSegmentServiceInterfaceServerThreadPtr_->join();
    }
    VisionSegmentServiceInterfaceServerThreadPtr_ = nullptr;
