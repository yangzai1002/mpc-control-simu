...
bool Sample::Init()
{
    	ApLogInterface::Instance()->Init("HwFr671RadarLeft");
...
void Sample::RadarPointCloudServiceInterfaceServerRun()
{
    ...
    while (workFlag_) {
		...
		AVOS_MSGPACK_PUBLISH("HwFr671RadarLeft",portName, "SensorRadarPointCloud" , static_cast<void *>(data.get()));
...
}void Sample::SensorRadarPacketServiceInterfaceServerRun()
{
    ...
    while (workFlag_) {
		...
		AVOS_MSGPACK_PUBLISH("HwFr671RadarLeft",portName, "SensorRadarPacket" , static_cast<void *>(data.get()));
